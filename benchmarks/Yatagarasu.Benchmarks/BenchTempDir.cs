...
///
/// 対策として、全ベンチの一時 DB を <see cref="Root"/>(%TEMP%\yatagarasu_bench\)
...
{
    /// <summary>
    /// 全ベンチ共通の一時 DB ルート(%TEMP%\yatagarasu_bench\)。
    /// 個別のランダム名ディレクトリをすべてこの配下に置くことで、起動時 /
    /// 終了時に 1 回の削除で残骸を回収できるようにしている。
    /// </summary>
    public static readonly string Root =
        Path.Combine(Path.GetTempPath(), "yatagarasu_bench");

    /// <summary>
    /// <paramref name="prefix"/> + ランダム suffix で <see cref="Root"/> 配下の
    /// パスを 1 つ返す。ディレクトリ実体は作らない(YatagarasuDatabase.Open など
