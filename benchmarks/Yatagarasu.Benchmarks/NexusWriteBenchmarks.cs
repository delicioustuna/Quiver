...
/// <summary>
/// 製品 API を通したNexus書き込みの費用を計測する。
/// アリティ 2 / 4 / 8 / 16 について、作成・プロパティ書き込み・削除の
/// 1 件あたり遅延と、作成の WAL バイト増幅 (binary Edge比) を測る。
/// さらに、1 Vertexが 10^3〜10^4 のNexusのメンバーである状態の
/// <c>DeleteVertex</c> カスケードのトランザクション時間・WAL・削除後整合性を記録する。
/// </summary>
/// <remarks>
/// WAL 増幅の判定は binary Edge作成比 <c>(1 + arity / 2)</c> 倍以内。
/// 高次数カスケードは数値ゲートを置かず、実測値と挙動 (デッドロックしないこと、
/// 削除後の整合性検査が 0 件) を記録する。
/// </remarks>
public static class NexusWriteBenchmarks
{
    ...
    {
        Console.WriteLine("=== Nexus write costs (product API) ===");
        Console.WriteLine(
            $"latencyOps={LatencyOps}/tx  walBatch={WalBatch}  " +
            "walGate=<=(1+arity/2)x binary edge");
        Console.WriteLine();

        long binaryWal = MeasureEdgeCreateWal(WalBatch);
        Console.WriteLine(
            $"binary edge create WAL: {binaryWal} bytes " +
        ...
        foreach (int arity in Arities)
        {
            ...
            long createWal = MeasureNexusCreateWal(arity, WalBatch);
    ...
    {
        string directory = BenchTempDir.Create($"nexus_create{arity}");
        Directory.CreateDirectory(directory);
        try
        {
            using var db = YatagarasuDatabase.Open(Path.Combine(directory, "graph.yata"));
    ...
    {
        string directory = BenchTempDir.Create($"nexus_prop{arity}");
        Directory.CreateDirectory(directory);
        try
        {
            using var db = YatagarasuDatabase.Open(Path.Combine(directory, "graph.yata"));
    ...
    {
        string directory = BenchTempDir.Create($"nexus_delete{arity}");
        Directory.CreateDirectory(directory);
        try
        {
            using var db = YatagarasuDatabase.Open(Path.Combine(directory, "graph.yata"));
    ...
    private static long MeasureNexusCreateWal(int arity, int itemCount)
    {
        string directory = BenchTempDir.Create($"nexus_createwal{arity}");
        Directory.CreateDirectory(directory);
    ...
    private static long MeasureEdgeCreateWal(int itemCount)
    {
        string directory = BenchTempDir.Create("nexus_edgewal");
        Directory.CreateDirectory(directory);
        string databasePath = Path.Combine(directory, "graph.yata");
        try
        {
    ...
    // ── 高次数 DeleteVertex カスケード ────────────────────────────────────────

    // hub は degree 個のNexusすべてで subject を担う。DeleteVertex は所属する
    // 全Nexusを同一トランザクションで削除する契約なので、その tx 時間・WAL・
    // 削除後の整合性を測る。RAG では Chunk や頻出エンティティが高次数になりうる。
    private static void RunDeleteVertexCascade(int degree)
    {
        string directory = BenchTempDir.Create($"nexus_cascade{degree}");
        Directory.CreateDirectory(directory);
        string databasePath = Path.Combine(directory, "graph.yata");
        try
        {
    ...
    private static NexusMember[] BuildMembers(int arity, VertexId[] vertices)
    {
    ...
    private static NexusId[] CommitCreates(YatagarasuDatabase db, NexusMember[] members, int count)
    {
    ...
    private static void WriteProperty(YatagarasuDatabase db, NexusId[] ids, PropertyValue value)
    {
        using var tx = db.BeginWriteTransaction();
        foreach (NexusId id in ids)
    ...
    private static YatagarasuDatabase OpenForWalMeasurement(string databasePath)
    {
        // 計測区間内の自動 checkpoint/truncate を止め、WAL file length 差分を
        // 実際の書き込み量に一致させる。
        var options = new YatagarasuDatabaseOptions { CheckpointThresholdBytes = 0 };
