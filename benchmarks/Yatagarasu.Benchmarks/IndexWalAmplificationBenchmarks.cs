...
public class IndexWalAmplificationBenchmarks
{
    ...
    {
        var walFile = Path.Combine(_dbPath, "graph.yata-wal");
...
public static class IndexWalAmplificationStandalone
{
    ...
    public static Result Run(string scenario, int entryCount)
    {
        ...
        try
        {
            ...
            using var db = YatagarasuDatabase.Open(System.IO.Path.Combine(dbPath, "graph.yata"), options);
            ...
            long walBytes = 0;
            var walFile = Path.Combine(dbPath, "graph.yata-wal");
