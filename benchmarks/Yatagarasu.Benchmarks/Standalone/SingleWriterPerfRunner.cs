...
/// commit p50 を同一プロセス・同一 workload で比較する性能ゲート。
/// </summary>
public static class SingleWriterPerfRunner
{
    ...
    private static double Measure(int readerCount, out long readerOperations)
    {
        ...
        try
        {
            ...
            for (int index = 0; index < readerCount; index++)
            {
                ...
                {
                    IsBackground = true,
                    Name = $"yatagarasu-snapshot-reader-{local}",
                };
                threads[index].Start();
            }
    ...
    private static VertexId Seed(YatagarasuDatabase db)
    {
    ...
    private static double MeasureWriter(YatagarasuDatabase db)
    {
    ...
    private static void CommitOne(YatagarasuDatabase db)
    {
        using var tx = db.BeginWriteTransaction();
