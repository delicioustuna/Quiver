...
/// 全量計測する。WAL は単一サイドカー <c>graph.yata-wal</c> ()。
///
/// 起動方法: <c>dotnet run -c Release --project benchmarks/Yatagarasu.Benchmarks -- --fts6 [chunkCount] [queryCount]</c>
/// </summary>
public static class Fts6SearchRunner
{
    ...
    public static int Run(int searchChunks, int queryCount)
    {
        ...
        // ── WAL amplification on a bounded sample ────────────────────────────────
        ...
        try
        {
            using var db = YatagarasuDatabase.Open(System.IO.Path.Combine(dirFt, "graph.yata"));
    ...
    private static long MeasureWal(ZipfVocabulary vocab, int chunkCount, bool withIndex, out double ingestMs)
    {
        ...
        try
        {
            using var db = YatagarasuDatabase.Open(
                System.IO.Path.Combine(dir, "graph.yata"),
    ...
    private static double IngestCorpus(YatagarasuDatabase db, ZipfVocabulary vocab, int chunkCount, int seed)
    {
    ...
    private static List<double> MeasureSearchLatencies(
        YatagarasuDatabase db, ZipfVocabulary vocab, int queryCount, int seed)
    {
        var rng = new Random(seed);
    ...
    {
        var wal = Path.Combine(dir, "graph.yata-wal");
