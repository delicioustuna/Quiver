...
//   if (args[0] == "--spike-a") return Yatagarasu.Benchmarks.Standalone.Dev.SpikeAPlanCompileRunner.Run();
...
namespace Yatagarasu.Benchmarks.Standalone.Dev;

/// <summary>
/// タスク A の Spike A0: クエリ DSL のクエリごと固定費 (~60µs) が
/// LogicalOptimizer.Optimize / PhysicalPlanner.Plan / traversal 構築 / 実行 の
/// どこに乗っているかを配分計測する。InternalsVisibleTo("Yatagarasu.Benchmarks") 経由で
/// LogicalOptimizer / PhysicalPlanner / GraphTraversal の internal を直接叩く。
///
/// 起動: <c>dotnet run --project benchmarks/Yatagarasu.Benchmarks -c Release -- --spike-a</c>
/// </summary>
public static class SpikeAPlanCompileRunner
{
    public static int Run()
    {
        ...
        {
            {
                using var db0 = YatagarasuDatabase.Open(Path.Combine(dir, "graph.yata"));
