...
using Yatagarasu.Benchmarks;
using Yatagarasu.Benchmarks.Standalone;

// ── ベンチ一時 DB の残骸を起動時に掃除する ──────────────────────────────
// 各ベンチは %TEMP%\yatagarasu_bench\ 配下にダミー DB を作り、その削除を BDN の
...
bool skipTempSweep = Environment.GetEnvironmentVariable("YATAGARASU_BENCH_SKIP_SWEEP") == "1";
if (!skipTempSweep)
{
...
// reader なし writer と 32 snapshot readers 併走時の commit p50 比較。
...
// Yatagarasu.Benchmarks.RegressionCheck はこの形式を読んで baselines/main.json と
