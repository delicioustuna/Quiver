...
public static class NexusTraversalBenchmarks
{
    ...
    private static bool RunDegree(int degree)
    {
        ...
        try
        {
            ...
            double chainP50;
            using (var db = YatagarasuDatabase.Open(databasePath))
            {
                using var tx = db.BeginReadTransaction();
                var g = tx.Query;
                chainP50 = MeasureP50(
    ...
    private static YatagarasuDatabase OpenWithView(string databasePath)
    {
    ...
    private static void AssertViewEngaged(YatagarasuDatabase db)
    {
        using var tx = db.BeginReadTransaction();
