...
public static class NexusWriteBenchmarks
{
    ...
    private static QuiverDatabase OpenForWalMeasurement(string databasePath)
    {
        // 計測区間内の自動 checkpoint/truncate を止め、WAL file length 差分を
        // 実際の書き込み量に一致させる。
        var options = new QuiverDatabaseOptions { CheckpointThresholdBytes = 0 };
        return QuiverDatabase.Open(databasePath, options);
    }

    private static void Checkpoint(QuiverDatabase db, string directory)
    {
