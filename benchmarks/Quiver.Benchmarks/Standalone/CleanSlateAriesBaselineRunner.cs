...
/// <summary>
/// Clean-slate redesign の次段 spike に向けて、現行 ARIES カーネル上の比較値を
/// standalone 形式でまとめて取得するランナー。
/// </summary>
public static class CleanSlateAriesBaselineRunner
{
    private const string RelationshipType = "LINK";
    private const string RelationshipScoreKey = "score";
    ...
    public static int Run(IReadOnlyList<string> args)
    {
        ...
        Console.WriteLine("=== Clean-slate ARIES baseline ===");
        Console.WriteLine(
        ...
        MeasureRelationshipBaseline(degree, traversalIterations);
        Console.WriteLine();
    ...
    private static void MeasureRelationshipBaseline(int degree, int traversalIterations)
    {
        Console.WriteLine("--- relationship row-path baseline ---");
        string dir = BenchTempDir.Create("clean_slate_rel");
        try
        {
            using var db = GraphDatabase.Open(Path.Combine(dir, "graph.quiver"));
            var shape = BuildRelationshipCorpus(db, degree);
            ...
            using (var read = db.BeginReadOnlyTransaction())
            {
                ...
                directLookupNs = MeasureDirectRelationshipLookup(read, shape.SecondHopRelationships);
                Console.WriteLine(
                    $"predicate_2hop, edges={shape.SecondHopRelationships.Length}, " +
                ...
                Console.WriteLine($"relationship_property_direct_lookup, ns_per_lookup={directLookupNs:F1}");
            }

            var update = MeasurePointUpdateCommits(
                db,
                shape.SecondHopRelationships,
                Math.Max(200, traversalIterations));
            Console.WriteLine(
                $"relationship_property_update_commit, commits={update.Count}, " +
                $"p50_us={update.P50Us:F2}, p95_us={update.P95Us:F2}");
            Console.WriteLine(
                "csv,relationship,predicate_2hop_p50_ms,predicate_2hop_p95_ms," +
                "direct_lookup_ns,update_commit_p50_us,update_commit_p95_us");
            Console.WriteLine(
                $"csv,relationship,{traversalP50Ms:F4},{traversalP95Ms:F4}," +
    ...
    private static RelationshipCorpus BuildRelationshipCorpus(GraphDatabase db, int degree)
    {
        var secondHopRelationships = new RelationshipId[degree * degree];
        int expectedMatches = 0;

        using var tx = db.BeginTransaction();
        var hub = tx.CreateNode("Hub");
        int relIndex = 0;
        for (int i = 0; i < degree; i++)
        {
            var mid = tx.CreateNode("Mid");
            tx.CreateRelationship(hub, mid, RelationshipType);
            for (int j = 0; j < degree; j++)
            {
                var leaf = tx.CreateNode("Leaf");
                var rel = tx.CreateRelationship(mid, leaf, RelationshipType);
                long score = (i + j) % 4 == 0 ? 1 : 0;
                tx.SetProperty(rel, RelationshipScoreKey, PropertyValue.FromInt64(score));
                secondHopRelationships[relIndex++] = rel;
                if (score == 1) expectedMatches++;
            }
        }
        tx.Commit();

        return new RelationshipCorpus(hub, secondHopRelationships, expectedMatches);
    }

    private static int CountPredicateMatches(IGraphTransaction tx, NodeId hub)
    {
        int count = 0;
        var first = tx.EnumerateRelationships(hub, Direction.Outgoing, RelationshipType);
        while (first.MoveNext())
        {
            var mid = first.Current.Target;
            var second = tx.EnumerateRelationships(mid, Direction.Outgoing, RelationshipType);
            while (second.MoveNext())
            {
                var value = tx.GetProperty(second.Current.Id, RelationshipScoreKey);
    ...
    private static double MeasureDirectRelationshipLookup(
        IGraphTransaction tx,
        IReadOnlyList<RelationshipId> relationshipIds)
    {
        const int SampleSize = 4096;
        int[] sample = new int[SampleSize];
        var rng = new Random(1234);
        for (int i = 0; i < sample.Length; i++)
            sample[i] = rng.Next(relationshipIds.Count);

        int cursor = 0;
        long sum = 0;
        long Lookup()
        {
            var rel = relationshipIds[sample[cursor++ & (SampleSize - 1)]];
            sum += tx.GetProperty(rel, RelationshipScoreKey).Int64Value;
            return sum;
        }
        ...
        int commits)
    {
        ...
        {
            var rel = relationshipIds[i % relationshipIds.Count];
            long started = Stopwatch.GetTimestamp();
            using (var tx = db.BeginTransaction())
            {
                tx.SetProperty(rel, RelationshipScoreKey, PropertyValue.FromInt64(10_000 + i));
                tx.Commit();
            }
    ...
    {
        Console.WriteLine("--- full-text ARIES baseline ---");
        ...
        int seed)
    {
        ...
        while (written < chunkCount)
        {
            ...
            for (int i = 0; i < batch; i++)
            {
                ...
                tx.SetProperty(node, "body", PropertyValue.FromString(MakeChunk(vocab, rng)));
            }
            tx.Commit();
            written += batch;
        }
    ...
    {
        Console.WriteLine("--- vector ARIES baseline ---");
    ...
    private readonly record struct RelationshipCorpus(
        NodeId Hub,
        RelationshipId[] SecondHopRelationships,
        int ExpectedMatches);
