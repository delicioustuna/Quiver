...
/// <summary>
/// Clean-slate CSR relationship の最小 spike。
/// 既存の永続 adjacency payload lane を immutable CSR base segment 近似として使い、
/// relationship property predicate 付き 2-hop 走査の上限性能を測る。
/// </summary>
public static class CleanSlateCsrRelationshipSpikeRunner
{
    ...
    public static int Run(IReadOnlyList<string> args)
    {
        ...
        Console.WriteLine("=== Clean-slate CSR relationship spike ===");
        Console.WriteLine(
        ...
        string dir = BenchTempDir.Create("clean_slate_csr_rel");
        try
        {
            string path = Path.Combine(dir, "graph.quiver");
            CsrCorpus corpus;
            using (var buildDb = GraphDatabase.Open(path))
            ...
            Console.WriteLine(
                "csv,csr_relationship,predicate_2hop_p50_ms,predicate_2hop_p95_ms," +
                "speedup,direct_lookup_ns,delta_overlay_update_ns,result");
            Console.WriteLine(
                $"csv,csr_relationship,{p50:F4},{p95:F4},{baselineP50Ms / p50:F2}," +
    ...
    private static CsrCorpus BuildCorpus(GraphDatabase db, int degree)
    {
        ...
        int nodeCount = 1 + degree + degree * degree;
        int firstHopEdges = degree;
        int secondHopEdges = degree * degree;
        int totalEdges = firstHopEdges + secondHopEdges;
        var sourceByRel = new long[totalEdges];
        var targetByRel = new long[totalEdges];
        var scoreByRel = new long[totalEdges];
        ...
        {
            long midNode = 1 + mid;
            AppendRel(loader, relId, 0, midNode, type, scoreKey, 0, sourceByRel, targetByRel, scoreByRel);
            relId++;
            ...
            for (int leaf = 0; leaf < degree; leaf++)
            {
                ...
                AppendRel(loader, relId, midNode, leafNode, type, scoreKey, score, sourceByRel, targetByRel, scoreByRel);
                if (score == 1)
                    expectedMatches++;
                relId++;
            }
    ...
    private static void AppendRel(
        ...
        long[] scoreByRel)
    {
        var relationshipId = new RelationshipId(relId);
        loader.AppendRelationship(relationshipId, new NodeId(source), new NodeId(target), type);
        loader.AppendRelationshipPayload(relationshipId, scoreKey, score);
        sourceByRel[relId] = source;
        targetByRel[relId] = target;
        scoreByRel[relId] = score;
    }

    private static int CountPredicateMatches(IGraphTransaction read, NodeId hub)
    {
    ...
    private static double MeasureLocatorLookup(CsrCorpus corpus)
    {
        ...
        for (int i = 0; i < sample.Length; i++)
            sample[i] = rng.Next(corpus.ScoreByRel.Length);

        int cursor = 0;
        long checksum = 0;
        long Lookup()
        {
            int rel = sample[cursor++ & (SampleSize - 1)];
            checksum += corpus.SourceByRel[rel];
            checksum += corpus.TargetByRel[rel];
            checksum += corpus.ScoreByRel[rel];
            return checksum;
        }
    ...
    private static double MeasureDeltaOverlayPointUpdate(CsrCorpus corpus)
    {
        ...
        long Update()
        {
            long rel = 1 + (cursor++ % Math.Max(1, corpus.SecondHopEdges));
            delta[rel] = cursor;
            return delta.Count;
        }
