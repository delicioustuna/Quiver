...
public class ColumnAggregationBenchmarks
{
    ...
    private IReadTransaction _colTx = null!;
    private IReadTransaction _rowTx = null!;
    ...
    public void Setup()
    {
        ...
        _colTx = _colDb.BeginReadTransaction();
        _rowTx = _rowDb.BeginReadTransaction();
    }

    private void Seed(string dir, bool withColumn)
    {
        using var db = QuiverDatabase.Open(System.IO.Path.Combine(dir, "graph.quiver"));
        _ = db.EditSchema(schema => schema.GetOrCreateLabel("N"));
        const int batch = 50_000;
        int written = 0;
        IWriteTransaction? tx = db.BeginWriteTransaction();
        try
        {
            for (int i = 0; i < VertexCount; i++)
            {
                ...
                if (++written >= batch)
                {
                    tx.Commit(); tx.Dispose(); tx = db.BeginWriteTransaction(); written = 0;
                }
            }
            tx!.Commit();
        }
    ...
    public long RowPathSum() => _rowTx.Query.Vertices().SumLong("v");

    [Benchmark(Description = "Column scan SumLong (dense MVCC column)")]
    public long ColumnScanSum() => _colTx.Query.Vertices().SumLong("v");
}
