...
/// <summary>
/// SourceGenerator が生成する型保存ハイパーエッジトラバーサル糖衣を実 DB に接続して
/// 検証する。型付き走査は型なし DSL と同じ論理オペレータへ委譲するため、
/// 両者の結果集合が一致することを単一・複数・省略可能ロール、同一ノード型の
/// 複数ロール、プロパティフィルタの各形状で確認する。
/// </summary>
public sealed class HyperedgeTypedTraversalTests : IDisposable
{
    private readonly string _dir;
    private readonly string _path;

    public HyperedgeTypedTraversalTests()
    {
    ...
    private static List<long> Ids(List<NodeId> ids) => ids.ConvertAll(id => (long)id.Value);

    private static List<long> Ids<T>(List<(NodeId Id, T Entity)> pairs) where T : IGraphNode<T>
    ...
    public void Single_role_typed_traversal_matches_untyped()
    {
        ...
        var typed = g.Nodes<Person>().Has(p => p.Name, "Alice")
            .EmploymentAsEmployee().Employer().ToListWithIds();
        var untyped = g.Node(alice)
            .Hyperedges("Employment", "Employee").Members("Employer").ToList();

        Ids(typed).Should().BeEquivalentTo(Ids(untyped));
        Ids(typed).Should().BeEquivalentTo(Ids(new List<NodeId> { acme }));
        // 型付き終端はエンティティを復元する。
    ...
    public void Multi_role_typed_traversal_matches_untyped()
    {
        ...
        // 複数メンバーロールは各メンバーノードを個別の行として放出する。
        var typed = g.Nodes<Person>().Has(p => p.Name, "A")
            .MeetingAsAttendees().Attendees().ToListWithIds();
        var untyped = g.Node(a)
            .Hyperedges("Meeting", "Attendee").Members("Attendee").ToList();

        Ids(typed).Should().BeEquivalentTo(Ids(untyped));
        Ids(typed).Should().BeEquivalentTo(Ids(new List<NodeId> { a, b, c }));

        // co-membership: 起点ノードを全ロールから除外する。
        var typedOthers = g.Nodes<Person>().Has(p => p.Name, "A")
            .MeetingAsAttendees().OtherAttendees().ToListWithIds();
        var untypedOthers = g.Node(a)
            .Hyperedges("Meeting", "Attendee").OtherMembers("Attendee").ToList();

        Ids(typedOthers).Should().BeEquivalentTo(Ids(untypedOthers));
        Ids(typedOthers).Should().BeEquivalentTo(Ids(new List<NodeId> { b, c }));
    }
    ...
    public void Optional_role_typed_traversal_matches_untyped()
    {
        ...
        var untyped = g.Node(a)
            .Hyperedges("Meeting", "Organizer").Members("Venue").ToList();

        Ids(typed).Should().BeEquivalentTo(Ids(untyped));
        Ids(typed).Should().BeEquivalentTo(Ids(new List<NodeId> { hq }));
    }

    [Fact]
    public void Same_node_type_in_multiple_roles_matches_untyped()
    {
        using var db = GraphDatabase.Open(_path);
        ...
        using (var tx = db.BeginTransaction())
        {
            ...
            // Subject と Object は同じノード型 (Person) の別ロール。逆向きの 1 件も
        ...
        var typedObjects = g.Nodes<Person>().Has(p => p.Name, "Alice")
            .StatementAsSubject().Object().ToListWithIds();
        var untypedObjects = g.Node(alice)
            .Hyperedges("Statement", "Subject").Members("Object").ToList();

        Ids(typedObjects).Should().BeEquivalentTo(Ids(untypedObjects));
        Ids(typedObjects).Should().BeEquivalentTo(Ids(new List<NodeId> { bob }));

        // Object ロールで参加する Statement の Subject メンバー (逆方向)。
        var typedSubjects = g.Nodes<Person>().Has(p => p.Name, "Alice")
            .StatementAsObject().Subject().ToListWithIds();
        var untypedSubjects = g.Node(alice)
            .Hyperedges("Statement", "Object").Members("Subject").ToList();

        Ids(typedSubjects).Should().BeEquivalentTo(Ids(untypedSubjects));
        Ids(typedSubjects).Should().BeEquivalentTo(Ids(new List<NodeId> { bob }));
    }

    [Fact]
    public void Hyperedge_property_filter_matches_untyped()
    {
        using var db = GraphDatabase.Open(_path);

        NodeId alice, acme, globex;
        using (var tx = db.BeginTransaction())
        {
            alice = Person.Insert(tx, new Person { Name = "Alice" });
            acme = Company.Insert(tx, new Company { Name = "Acme" });
        ...
        // ハイパーエッジプロパティの式ツリーフィルタは型なし Has(key, value) と同じ述語になる。
        var typed = g.Nodes<Person>().Has(p => p.Name, "Alice")
            .EmploymentAsEmployee().Has(e => e.Title, "Engineer").Employer().ToListWithIds();
        var untyped = g.Node(alice)
            .Hyperedges("Employment", "Employee").Has("Title", "Engineer").Members("Employer").ToList();

        Ids(typed).Should().BeEquivalentTo(Ids(untyped));
        Ids(typed).Should().BeEquivalentTo(Ids(new List<NodeId> { acme }));

        // 型付きハイパーエッジ終端はエンティティを復元できる。
        var employments = g.Nodes<Person>().Has(p => p.Name, "Alice")
            .EmploymentAsEmployee().Has(e => e.Year, 2022).ToList();
        employments.Should().ContainSingle().Which.Title.Should().Be("Manager");
    }
}

// Subject と Object が同じノード型 (Person) を束縛するハイパーエッジ。
// 同一ノード型の複数ロールをロール指定で区別できることの検証に使う。
[Hyperedge("Statement")]
