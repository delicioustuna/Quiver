...
/// <summary>
/// ハイパーエッジを node / relationship と同じプロパティエンティティとして扱えることを検証する。
/// スカラ / 文字列 / バイト列 / float 配列、inline と overflow の相互遷移、Set cardinality、
/// スナップショット分離、再オープン後の永続性をカバーする。
/// </summary>
public sealed class HyperedgePropertyTests : IDisposable
{
    private readonly string _dir;
    private readonly string _path;

    public HyperedgePropertyTests()
    {
    ...
    private static HyperedgeId Seed(IGraphTransaction tx)
    {
        var a = tx.CreateNode("A");
        var b = tx.CreateNode("B");
        return tx.CreateHyperedge("Fact", [new("Subject", a), new("Object", b)]);
    }
