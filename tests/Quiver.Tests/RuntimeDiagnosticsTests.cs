...
public sealed class RuntimeDiagnosticsTests : IDisposable
{
    private readonly string _directory = Path.Combine(
        Path.GetTempPath(),
        "quiver_runtime_diagnostics_" + Guid.NewGuid().ToString("N"));
    ...
    public void Snapshot_diagnostics_and_observable_metrics_report_the_oldest_reader()
    {
        ...
        using var database = QuiverDatabase.Open(Path.Combine(_directory, "graph.quiver"));
        ...
        measurements.Values("quiver.snapshot.active.count")
            .Should().Contain(value => value >= 1);
        measurements.Values("quiver.snapshot.oldest.age")
    ...
    public async Task Writer_contention_records_count_and_actual_wait_duration()
    {
        ...
        using var database = QuiverDatabase.Open(
            Path.Combine(_directory, "writer.quiver"),
        ...
        measurements.Values("quiver.writer.contention.count")
            .Should().Contain(value => value >= 1);
        measurements.Values("quiver.writer.wait.duration")
            .Should().Contain(value => value > 0);
    }
    ...
    public void Maintenance_metrics_balance_rebuild_and_garbage_collection_state()
    {
        ...
        using var database = QuiverDatabase.Open(
            Path.Combine(_directory, "maintenance.quiver"));
        try
        {
            ...
            measurements.Values("quiver.maintenance.rebuild.active")
                .Should().Contain(1);
            releaseRebuild.Set();
            ...
            measurements.Values("quiver.maintenance.gc.active")
                .Should().ContainInOrder(1, -1);
        }
        finally
        {
    ...
    private sealed class MeasurementCapture : IDisposable
    {
        ...
        internal MeasurementCapture()
        {
            ...
            {
                if (instrument.Meter.Name == QuiverTelemetry.MeterName)
