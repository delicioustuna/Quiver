...
public sealed class GraphJsonExportTests
{
    ...
    public void Export_writes_type_faithful_graph_json()
    {
        ...
        root.GetProperty("format").GetString().Should().Be("yatagarasu-graph");
    ...
    public void Database_instance_id_survives_reopen_and_snapshot_copy()
    {
        string directory = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_export_identity_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        string sourcePath = Path.Combine(directory, "source.yata");
        string snapshotPath = Path.Combine(directory, "snapshot.yata");
        try
        {
            DatabaseInstanceId original;
            using (var db = YatagarasuDatabase.Open(sourcePath))
            {
                db.TryGetDatabaseInstanceId(out original).Should().BeTrue();
                db.CreateSnapshot(snapshotPath);
            }

            using (var reopened = YatagarasuDatabase.Open(sourcePath))
            {
    ...
    public void Legacy_database_read_export_does_not_create_identity_metadata()
    {
        string directory = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_export_legacy_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        string sourcePath = Path.Combine(directory, "legacy.yata");
        try
        {
    ...
    public void Legacy_identity_creation_does_not_join_an_active_writer_write_set()
    {
        string directory = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_export_identity_lease_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        string sourcePath = Path.Combine(directory, "legacy.yata");
        try
        {
            ...
            using (var db = YatagarasuDatabase.Open(sourcePath, new YatagarasuDatabaseOptions
            {
