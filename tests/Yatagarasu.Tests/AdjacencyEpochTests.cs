...
public sealed class AdjacencyEpochTests : IDisposable
{
    ...
    private YatagarasuDatabase? _db;

    public AdjacencyEpochTests()
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_pw14_" + Guid.NewGuid().ToString("N"));
    }
    ...
    public void Compact_adjacency_fail_fast_reports_writer_busy_when_writer_is_active()
    {
        ...
        _db = YatagarasuDatabase.Open(
            System.IO.Path.Combine(_dir, "graph.yata"),
    ...
    private Yatagarasu.Storage.SingleFileContainer NewContainer()
    {
        Directory.CreateDirectory(_dir);
        var c = new Yatagarasu.Storage.SingleFileContainer(
            Path.Combine(_dir, "epoch_" + Guid.NewGuid().ToString("N")[..8] + ".yata"));
        _containers.Add(c);
        return c;
    }
    private readonly List<Yatagarasu.Storage.SingleFileContainer> _containers = new();

    [Fact]
    public void AdjacencyEpoch_round_trip_preserves_tombstones_and_epoch()
    {
        var container = NewContainer();
        var tenant = container.OpenTenant(AdjacencyContainer.EpochTenant, Yatagarasu.Storage.PageKind.Header);
