...
/// <summary>実際の <c>YatagarasuDatabase</c> を使って <c>EdgeScanExpandOperator</c> を検証する。</summary>
public sealed class EdgeScanExpandOperatorTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_pw17_" + Guid.NewGuid().ToString("N"));
...
internal sealed class FixedVertexListOperatorForPw17 : IPhysicalOperator
{
    ...
    public void Open(Yatagarasu.Transactions.ITransaction tx) { _index = -1; }
