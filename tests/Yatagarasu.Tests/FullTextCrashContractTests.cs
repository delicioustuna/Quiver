...
/// <summary>
/// 全文インデックスの永続性契約を <see cref="YatagarasuDatabase"/> と
...
public sealed class FullTextCrashContractTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_fts6_crash_" + Guid.NewGuid().ToString("N"));
    ...
    private YatagarasuDatabase Open() => YatagarasuDatabase.Open(_path);
    ...
    /// カタログからインデックスを再構築する。
    /// </summary>
    private YatagarasuDatabase OpenAndCreateIndex()
    {
        var db = Open();
    ...
    private static void Kill(YatagarasuDatabase db)
    {
    ...
    private static VertexId Ingest(YatagarasuDatabase db, string body)
    {
    ...
    private static List<VertexId> Search(YatagarasuDatabase db, string query, int k = 10)
    {
