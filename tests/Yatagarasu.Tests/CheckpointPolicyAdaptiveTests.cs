...
public sealed class CheckpointPolicyAdaptiveTests : IDisposable
{
    private readonly string _dir = Path.Combine(
        Path.GetTempPath(), "yatagarasu_ft28_" + Guid.NewGuid().ToString("N"));
    ...
    /// </summary>
    private long RunPerTxWorkload(YatagarasuDatabaseOptions options, int commits)
    {
    ...
    public void Fixed_policy_is_default_and_preserves_legacy_behavior()
    {
        ...
        using var db = YatagarasuDatabase.Open(System.IO.Path.Combine(_dir, "graph.yata"), opts);
    ...
    public void Adaptive_policy_keeps_data_correct_across_reopen()
    {
        ...
        using (var db2 = YatagarasuDatabase.Open(System.IO.Path.Combine(_dir, "graph.yata"), opts))
        {
