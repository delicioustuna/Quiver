...
public sealed class BooleanSearchTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_bool_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var match = AddDoc("embedded yatagarasu database engine");
        AddDoc("embedded yatagarasu visualization tool");
        AddDoc("database only");

        var results = Search("embed* AND database");
    ...
    {
        var match = AddDoc("embedded yatagarasu engine");
        AddDoc("embedded yatagarasu database");

        var results = Search("embed* NOT database");
