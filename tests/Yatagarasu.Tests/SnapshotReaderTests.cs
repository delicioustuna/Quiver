...
public sealed class SnapshotReaderTests : IDisposable
{
    ...
    {
        _directory = Path.Combine(Path.GetTempPath(), "yatagarasu_snapshot_readers_" + Guid.NewGuid().ToString("N"));
        _database = YatagarasuDatabase.Open(Path.Combine(_directory, "graph.yata"));
    }
