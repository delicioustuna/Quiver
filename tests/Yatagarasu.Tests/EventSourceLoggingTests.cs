...
public sealed class EventSourceLoggingTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_eventsource_" + Guid.NewGuid().ToString("N"));
    ...
    {
        YatagarasuEventSource.Log.OldSnapshotDetected(
            activeCount: 2,
    ...
    private sealed class CapturingEventListener : EventListener
    {
        ...
        {
            if (eventSource.Name == YatagarasuEventSource.EventSourceName)
