...
public sealed class FullTextConcurrencyTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_fts6_conc_" + Guid.NewGuid().ToString("N"));
        _db = YatagarasuDatabase.Open(System.IO.Path.Combine(_dir, "graph.yata"), new YatagarasuDatabaseOptions
        {
