...
public sealed class LogicalMutationTests : IDisposable
{
    ...
    {
        var d = Path.Combine(Path.GetTempPath(), "yatagarasu_logical_" + Guid.NewGuid().ToString("N"));
        _tempDirs.Add(d);
        return d;
    }

    private static YatagarasuDatabase OpenWithSink(string dir, ILogicalMutationSink sink)
        => YatagarasuDatabase.Open(System.IO.Path.Combine(dir, "graph.yata"), new YatagarasuDatabaseOptions { LogicalMutationSink = sink });
    ...
    public void Sink_is_silent_when_no_sink_configured()
    {
        ...
        using var db = YatagarasuDatabase.Open(System.IO.Path.Combine(NewDir(), "graph.yata"));
        using var tx = db.BeginWriteTransaction();
    ...
    public void Replay_rebuilds_graph_state_from_logical_stream()
    {
        ...
        using var target = YatagarasuDatabase.Open(System.IO.Path.Combine(targetDir, "graph.yata"));
    ...
    public void Replay_preserves_edge_removal_and_set_property_mutations()
    {
        ...
        using var target = YatagarasuDatabase.Open(
            System.IO.Path.Combine(NewDir(), "graph.yata"));
