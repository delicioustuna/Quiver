...
/// <see cref="YatagarasuDatabase"/> のオープンおよび破棄との連携を確認する。
/// </summary>
public sealed class AutoVacuumWorkerTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_op7_" + Guid.NewGuid().ToString("N"));
    }
    ...
    // ---- YatagarasuDatabase 配線 ----

    [Fact]
    public void YatagarasuDatabase_disabled_AutoVacuum_does_not_auto_run()
    {
    ...
    [Fact]
    public void YatagarasuDatabase_enabled_AutoVacuum_reclaims_dead_versions_in_background()
    {
    ...
    [Fact]
    public void YatagarasuDatabase_Dispose_stops_worker_cleanly()
    {
        var options = new YatagarasuDatabaseOptions
        {
            AutoVacuum = true,
        ...
        var db = YatagarasuDatabase.Open(System.IO.Path.Combine(_dir, "graph.yata"), options);
        using (var tx = db.BeginWriteTransaction())
        {
            tx.CreateVertex("Person");
            tx.Commit();
