...
public sealed class YatagarasuDatabaseTests : IDisposable
{
    private readonly string _dir;
    private readonly YatagarasuDatabase _db;

    public YatagarasuDatabaseTests()
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_test_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var path = Path.Combine(_dir, "edge_locator_reopen.yata");
        EdgeId stamped;
    ...
    {
        var path = Path.Combine(_dir, "edge_locator_reuse.yata");
        using var db = YatagarasuDatabase.Open(path);
    ...
    public void Committed_data_survives_data_file_loss_via_wal_replay()
    {
        ...
        var dir = Path.Combine(Path.GetTempPath(), "yatagarasu_crash_" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            // 単一ファイルコンテナでは、すべてのコアデータを graph.yata に格納する。
            byte[] dataSnap = File.ReadAllBytes(Path.Combine(dir, "graph.yata"));

            VertexId aliceId;

            // データを書き込んでコミットする。WAL はフラッシュされるが、バッファープールは未反映でもよい。
            // 正常終了時は Dispose が全ページのフラッシュと WAL の削除を行うため、
            // graph.yata が確定して WAL が消える。WAL replay 経路を検証するため、未コミットの tx を
            // 1 つ開いたまま Dispose して「クラッシュ (ActiveCount>0 → WAL 非削除)」を模擬する。
            {
                var db = YatagarasuDatabase.Open(System.IO.Path.Combine(dir, "graph.yata"));
                using (var tx = db.BeginWriteTransaction())
                {
            ...
            File.WriteAllBytes(Path.Combine(dir, "graph.yata"), dataSnap);
    ...
    public void Match_typed_Out_and_Load_returns_entity()
    {
        ...
        var g = tx.Query;
        var p = Yatagarasu.Api.Match.GraphPattern.Vertex("p", "Person");
        var q = Yatagarasu.Api.Match.GraphPattern.Vertex("q", "Person");
    ...
    public void MatchQuery_AsCursor_streams_results()
    {
        ...
        using var rtx = _db.BeginReadTransaction();
        var g = rtx.Query;
        var p = Yatagarasu.Api.Match.GraphPattern.Vertex("p", "StreamPerson");
        var q = Yatagarasu.Api.Match.GraphPattern.Vertex("q", "StreamPerson");
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "yatagarasu_excl_" + Guid.NewGuid().ToString("N"));
        try
        {
            using var db = YatagarasuDatabase.Open(
                Path.Combine(dir, "g.yata"),
                new YatagarasuDatabaseOptions { WriterContentionMode = WriterContentionMode.FailFast });
