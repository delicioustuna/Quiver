...
public sealed class TransactionRuntimeContractTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_tx_contract_" + Guid.NewGuid().ToString("N"));
    ...
    {
        using var db = YatagarasuDatabase.Open(_path, new YatagarasuDatabaseOptions
        {
