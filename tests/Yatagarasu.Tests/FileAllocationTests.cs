...
public sealed class FileAllocationTests : IDisposable
{
    private readonly string _directory =
        Path.Combine(Path.GetTempPath(), "yatagarasu_allocation_" + Guid.NewGuid().ToString("N"));

    [Fact]
    public void Database_options_control_initial_allocation_and_growth_cap()
    {
        string path = Path.Combine(_directory, "graph.yata");
        ...
        using var database = YatagarasuDatabase.Open(path, options);
