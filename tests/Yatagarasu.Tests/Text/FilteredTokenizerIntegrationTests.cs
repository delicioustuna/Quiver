...
namespace Yatagarasu.Tests.Text;
...
public sealed class FilteredTokenizerIntegrationTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_filt_" + Guid.NewGuid().ToString("N"));
    ...
    {
        string path = Path.Combine(_dir, "reopen.yata");
    ...
    {
        string path = Path.Combine(_dir, "variant-reopen.yata");
        using (var database = YatagarasuDatabase.Open(path))
        {
            database.EditSchema(schema => schema.CreateIndex(
                new FullTextIndexDefinition(
