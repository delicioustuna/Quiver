...
/// <summary>
/// <c>YatagarasuDatabase</c> 経由で MVCC のスナップショット分離を確認する
/// エンドツーエンドテスト。
///
/// <para>
...
public sealed class SnapshotVisibilityTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "yatagarasu_mvcc_" + Guid.NewGuid().ToString("N"));
    ...
    public void Reopen_preserves_visibility_for_pre_committed_data()
    {
        ...
        using var reopened = YatagarasuDatabase.Open(System.IO.Path.Combine(_dir, "graph.yata"));
