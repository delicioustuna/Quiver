    ...
    : GraphStorageBackendCrashContractTests
{
    ...
    // binary backend は単一ファイル *.yata。コンテナは <dir>/graph.yata、
    // WAL は <dir>/graph.yata-wal。fault injector の LatestWalSegment / sidecar 削除はこの前提。
    protected override string DatabasePath
        => System.IO.Path.Combine(DatabaseDirectory, "graph.yata");
    ...
    public void KillAcrossWalSegmentRoll_recovers()
    {
        var dir = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_crash_walroll_" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                IGraphStorageBackend? backend = factory.Open(System.IO.Path.Combine(dir, "graph.yata"), opts);
                using (var tx = backend.BeginWriteTransaction())
                {
            ...
            using var reopened = factory.Open(System.IO.Path.Combine(dir, "graph.yata"), opts);
            using var rtx = reopened.BeginReadTransaction();
    ...
    public void EmptyDirectory_recovers_with_empty_FileRegistry_pass()
    {
        var dir = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_crash_emptydir_" + Guid.NewGuid().ToString("N"));
        try
        {
            Directory.CreateDirectory(dir);
            using var backend = new BinaryGraphStorageBackendFactory()
                .Open(System.IO.Path.Combine(dir, "graph.yata"), new YatagarasuDatabaseOptions());
            using var tx = backend.BeginWriteTransaction();
    ...
    /// <see cref="Yatagarasu.Index.IIndexManager.FlushAll"/> がなければ index file は永続化されず、
    ...
    public void CommittedIndexEntry_survives_checkpoint_then_kill()
    {
        var dir = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_ft18_chkpt_" + Guid.NewGuid().ToString("N"));
        try
        {
            var factory = new BinaryGraphStorageBackendFactory();
            // 2 回目の commit で checkpoint を強制する小さな閾値を使う。
            var opts = new YatagarasuDatabaseOptions { CheckpointThresholdBytes = 1 };
    ...
    /// PropertyTypeFlags) は graph.yata 内の専用テナントに同居する。よってファイル存在の
    ...
    public void IndexCatalog_persists_fileKind_across_kill()
    {
        ...
        // 索引は graph.yata に同居するため、独立した .idx / .fileKinds は作られない。
        File.Exists(Path.Combine(DatabaseDirectory, "indexes", ".fileKinds")).Should().BeFalse(
            " では索引カタログは graph.yata 内テナントに同居し別ファイルを作らない");
    ...
    ///
    /// checkpoint 誘発手段: <see cref="YatagarasuDatabaseOptions.CheckpointThresholdBytes"/> = 1
    ...
    private void RunCheckpointKillScenario(CheckpointPhase killAt)
    {
        var dir = Path.Combine(
            Path.GetTempPath(),
            $"yatagarasu_ft21_{killAt}_" + Guid.NewGuid().ToString("N"));
        try
        {
            var factory = new BinaryGraphStorageBackendFactory();
            var opts = new YatagarasuDatabaseOptions { CheckpointThresholdBytes = 1 };
        ...
        bool expectAdjacencyView)
    {
        ...
        var dir = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_adj_compact_crash_" + Guid.NewGuid().ToString("N"));
        var path = Path.Combine(dir, "graph.yata");

        try
        {
            PropertyKeyId weightKey;
            using (var seed = YatagarasuDatabase.Open(path))
            {
    ...
    {
        // WAL は単一サイドカー graph.yata-wal。
        var walPath = Path.Combine(DatabaseDirectory, "graph.yata-wal");
