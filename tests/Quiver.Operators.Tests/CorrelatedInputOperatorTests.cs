...
public class CorrelatedInputOperatorTests
{
    ...
    {
        using var op = new CorrelatedInputOperator();
        op.Bind(new TupleSlot { Type = TupleSlotType.NodeId, LongValue = 42 });
        op.Open(null!);
    ...
    {
        using var op = new CorrelatedInputOperator();
        op.Bind(new TupleSlot { Type = TupleSlotType.NodeId, LongValue = 1 });
        op.Open(null!);
        op.MoveNext().Should().BeTrue();
        op.MoveNext().Should().BeFalse();

        op.Bind(new TupleSlot { Type = TupleSlotType.NodeId, LongValue = 2 });
        op.Open(null!);
        op.MoveNext().Should().BeTrue();
