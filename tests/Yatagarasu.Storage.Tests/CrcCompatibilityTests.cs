...
using YatagarasuCrc32 = Yatagarasu.Core.Crc32;

namespace Yatagarasu.Storage.Tests;

public sealed class CrcCompatibilityTests : IDisposable
{
    private const string DatabaseFixtureName = "crc-golden.yata";
    private const string WalFixtureName = "crc-golden.yata-wal";

    private readonly string _tempDirectory =
        Path.Combine(Path.GetTempPath(), "YatagarasuCrcTests_" + Guid.NewGuid().ToString("N"));
    ...
    public void Current_family_writer_round_trips_and_checksums()
    {
        ...
        string databasePath = Path.Combine(_tempDirectory, "roundtrip.yata");
        string walPath = Path.Combine(_tempDirectory, "roundtrip.yata-wal");
    ...
    public void Reference_crc_matches_known_vector()
    {
        ...
        YatagarasuCrc32.HashToUInt32("123456789"u8).Should().Be(0xCBF4_3926u);
    }
    ...
    public void One_shot_crc_matches_reference_across_lengths()
    {
        ...
        {
            YatagarasuCrc32.HashToUInt32(bytes.AsSpan(0, length))
    ...
    public void Incremental_crc_matches_reference_across_chunk_boundaries()
    {
        ...
        {
            var actual = new YatagarasuCrc32();
    ...
    {
        string path = Path.Combine(_tempDirectory, "generated.yata");
    ...
    {
        string path = Path.Combine(_tempDirectory, "generated.yata-wal");
