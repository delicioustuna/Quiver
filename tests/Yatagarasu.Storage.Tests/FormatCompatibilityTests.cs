...
{
    private const string PreviousReleaseFixtureName = "v0.5.0-current.yata";
    private const byte OptionalExtensionTenantId = 0x3E;

    private readonly string _directory = Path.Combine(
        Path.GetTempPath(),
        "yatagarasu-format-compatibility-" + Guid.NewGuid().ToString("N"));
    ...
    {
        string path = CreateCurrentDatabase("unknown-extension.yata");
        ...
        Action open = () => YatagarasuDatabase.Open(path);
    ...
    {
        string path = CreateCurrentDatabase("unsupported-family.yata");
        byte[] bytes = File.ReadAllBytes(path);
        bytes[9] = 1;
        File.WriteAllBytes(path, bytes);
        byte[] beforeOpen = File.ReadAllBytes(path);
    ...
    {
        string path = CreateCurrentDatabase("unknown-record.yata");
    ...
    {
        string path = CreateCurrentDatabase("optional-extension.yata");
