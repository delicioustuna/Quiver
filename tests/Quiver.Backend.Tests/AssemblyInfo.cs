...
// この assembly の crash-contract / chaos テストは、テスト専用のグローバル静的
...
// commit が checkpoint を誘発し、注入例外が別 backend で発火する競合が起きる。
// sidecar 分の per-tx WAL 量が増えて checkpoint 発火頻度が上がると表面化するため、
