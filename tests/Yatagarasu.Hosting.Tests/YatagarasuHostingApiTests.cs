// Yatagarasu.Samples.Hosting (ASP.NET Core minimal API) を WebApplicationFactory<Program>
...
/// データベース状態を共有しない。
/// </summary>
public sealed class YatagarasuHostingApiTests : IDisposable
{
    private readonly string _dataDir;
    private readonly YatagarasuWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public YatagarasuHostingApiTests()
    {
        _dataDir = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_hosting_tests_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_dataDir);
        _factory = new YatagarasuWebApplicationFactory(_dataDir);
    ...
    public async Task GET_root_returns_endpoint_listing()
    {
        ...
        json.Should().Contain("Yatagarasu");
...
/// </summary>
internal sealed class YatagarasuWebApplicationFactory : WebApplicationFactory<Program>
{
    private readonly string _dataDir;

    internal YatagarasuWebApplicationFactory(string dataDir) => _dataDir = dataDir;
    ...
    protected override IHost CreateHost(IHostBuilder builder)
    {
        builder.ConfigureHostConfiguration(cfg =>
        {
            ...
            {
                ["Yatagarasu:DataDirectory"] = _dataDir,
            });
        });
