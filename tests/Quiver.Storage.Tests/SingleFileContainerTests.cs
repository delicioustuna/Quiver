...
public class SingleFileContainerTests : IDisposable
{
    ...
    public void Container_ProducesSingleFile()
    {
        using (var c = new SingleFileContainer(DbFile()))
        {
            ...
            WriteMarker(t, t.AllocatePage(PageKind.NodeRecord), 42);
        }
    ...
    {
        using var c = new SingleFileContainer(DbFile());
        var t = c.OpenTenant(7, PageKind.NodeRecord);
        t.PageCount.Should().Be(1); // 論理 page 0 予約のみ

        var p1 = t.AllocatePage(PageKind.NodeRecord);
        p1.Value.Should().Be(1);
        t.PageCount.Should().Be(2);

        var p2 = t.AllocatePage(PageKind.NodeRecord);
    ...
    {
        using var c = new SingleFileContainer(DbFile());
        var t1 = c.OpenTenant(1, PageKind.NodeRecord);
        var t2 = c.OpenTenant(2, PageKind.RelationshipRecord);

        // 両テナントとも論理 page 1 を持つが、物理ページは別。
        var a = t1.AllocatePage(PageKind.NodeRecord);
        var b = t2.AllocatePage(PageKind.RelationshipRecord);
        a.Value.Should().Be(1);
        b.Value.Should().Be(1);
    ...
    {
        using var c = new SingleFileContainer(DbFile());
        var first = c.OpenTenant(3, PageKind.NodeRecord);
        var second = c.OpenTenant(3, PageKind.NodeRecord);
    ...
    public void Data_PersistsAcrossReopen()
    {
        ...
        using (var c = new SingleFileContainer(path))
        {
            var t1 = c.OpenTenant(1, PageKind.NodeRecord);
            var t2 = c.OpenTenant(2, PageKind.RelationshipRecord);
            p1 = t1.AllocatePage(PageKind.NodeRecord);
            p2 = t2.AllocatePage(PageKind.RelationshipRecord);
    ...
    public void FreePage_ReusesLogicalId()
    {
        using var c = new SingleFileContainer(DbFile());
        var t = c.OpenTenant(1, PageKind.NodeRecord);
        var a = t.AllocatePage(PageKind.NodeRecord); // 1
        var b = t.AllocatePage(PageKind.NodeRecord); // 2
        var cc = t.AllocatePage(PageKind.NodeRecord); // 3
        a.Value.Should().Be(1);
        b.Value.Should().Be(2);
    ...
    public void PageTableChaining_AcrossManyPages_PersistsAcrossReopen()
    {
        ...
        {
            var t = c.OpenTenant(1, PageKind.NodeRecord);
            PageId first = t.AllocatePage(PageKind.NodeRecord);
            firstLogical = first.Value;
            WriteMarker(t, first, 7777);
            PageId last = first;
            for (int i = 1; i < n; i++)
                last = t.AllocatePage(PageKind.NodeRecord);
