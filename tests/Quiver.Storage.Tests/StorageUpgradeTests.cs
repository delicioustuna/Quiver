...
public sealed class StorageUpgradeTests : IDisposable
{
    private readonly string _directory = Path.Combine(
        Path.GetTempPath(),
        "quiver-storage-upgrade-" + Guid.NewGuid().ToString("N"));
    ...
    {
        string sourcePath = CreateCurrentDatabase("current.quiver");
        byte[] before = File.ReadAllBytes(sourcePath);
        DateTime lastWriteBefore = File.GetLastWriteTimeUtc(sourcePath);

        StorageUpgradeResult result = QuiverDatabase.UpgradeStorage(sourcePath);
    ...
    {
        string sourcePath = CreateVersionOneFixture("unsupported.quiver");
        byte[] before = File.ReadAllBytes(sourcePath);

        Action act = () => QuiverDatabase.UpgradeStorage(sourcePath);
    ...
    {
        string sourcePath = CreateCurrentDatabase("open.quiver");
        using var database = QuiverDatabase.Open(sourcePath);
    ...
    {
        string sourcePath = Path.Combine(_directory, "truncated.quiver");
    ...
    {
        string sourcePath = CreateCurrentDatabase("bad-checksum.quiver");
    ...
    public void Injected_failure_never_loses_source_and_retry_converges(int boundaryValue)
    {
        ...
        string sourcePath = CreateVersionOneFixture($"fault-{failureBoundary}.quiver");
    ...
    {
        string sourcePath = CreateVersionOneFixture("without-backup.quiver");
    ...
    {
        string sourcePath = CreateVersionOneFixture("lost-target.quiver");
        byte[] original = File.ReadAllBytes(sourcePath);
    ...
    {
        var failureBoundary = (StorageUpgradeBoundary)boundaryValue;
        string sourcePath = CreateVersionOneFixture($"corrupt-target-{failureBoundary}.quiver");
        byte[] original = File.ReadAllBytes(sourcePath);
    ...
    {
        string sourcePath = CreateVersionOneFixture("failed-step.quiver");
        byte[] original = File.ReadAllBytes(sourcePath);
    ...
    private sealed class TestUpgradeStep : IStorageUpgradeStep
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using var database = QuiverDatabase.Open(targetFilePath);
            using var transaction = database.BeginWriteTransaction();
