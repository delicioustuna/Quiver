# Quiver 運用ガイド (Operations Cookbook)

このディレクトリは **Quiver を埋め込んだアプリケーションを本番運用する人** に向けた実務ドキュメント集。
「どう API を呼ぶか」を扱う [docs/cookbook.md](../cookbook.md) や [docs/api/](../api/) とは目的が異なり、
ここでは **「どう設定すれば速いか」「壊れた DB をどう直すか」「何ができないか」** を扱う。

| 章 | いつ読むか |
|---|---|
| [01_quickstart.md](01_quickstart.md) | 初めて Quiver を埋め込む。最小構成で 1 つの DB を立ち上げ、最初の commit を通すまで。 |
