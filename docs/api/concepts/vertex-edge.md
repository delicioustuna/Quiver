...
Yatagarasu は有向プロパティグラフを採用する。Vertexはラベルを 1 つ持ち、複数のプロパティをキーバリュー形式で保持する。Edgeは型を 1 つ持ち、両端Vertex (`Source` / `Target`) と複数のプロパティを保持する。Nexusは型とロール付きの複数Vertexメンバーを持つn項関係であり、作成時に確定するメンバー集合と複数のプロパティを保持する。
