...
BR と MT の独立検証は [数理ユースケース横断 spike](math-usecases-spike.md) に記録した。

BR-1 の線形性と MT solver の小規模厳密性は通過したが、BR-2 の共有コスト契約と MT の budget 付き結果契約が未決である。

この結果だけでは BR または MT の本流着手を承認しない。
