...
- **[実装時判明・判断記録 (2026-07-03)]**: 既定構築パラメタ (M=16/Mmax0=32/efC=200) の実測 true
  recall@10 は **0.825** で、目標 0.95 に届かない。既定値は黙って変えない方針 (VP-2 判断ポイントと
  同旨) のため、ゲートを**二段構成**に変更した: (1) **default ゲート** = 既定構成の劣化監視 (床値
