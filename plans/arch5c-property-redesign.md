...
- **Phase 3 — property inline 化** ✅: 小/固定値を node record version へ inline、可変長(>255B)は overflow チェーン (PropertyStore) フォールバック。`GraphTransaction` + operator/query 層の get/has/set/remove/enumerate を inline+overflow へ。vacuum を版チェーン prune 対応へ。
  - **3a** (a2798ec): node MVCC(xmin/xmax) を sidecar→heap version へ再内包。sidecar は Generation+SSN(Pstamp/Sstamp)+commit 高水位へ縮小。
  - **3b** (7bb86bb): InlinePropertyCodec + VersionedNodeStore の inline get/has/set/remove + heap.AppendOrReplaceHead (同一tx in-place / 他tx copy-on-write)。純加算。
  - **3c** (1c699c2): INodeStore に inline メソッド追加 → TxNodeStore(lock+SSN ラップ)/VersionedNodeStore(実装)/NodeStore(graceful degrade=全overflow)。PropertyEnumerator を inline→overflow 二段列挙へ拡張(公開API不変)。GraphTransaction + PropertyLookupOperator + ClientPredicates + GraphStats + GraphEngineAdapter を結合列挙子へ。inline read も SSN read-set 記録。**BlobStore.Free のロックバグ修正** (read pin 保持中 write → LockRecursionException、overflow vacuum で顕在化)。
  - **3d**: vacuum で live ノードの dead 旧版 (property 更新の copy-on-write 残骸) を `heap.PruneDeadVersions` で回収。
  - 設計: MVCC/SSN は **node 粒度**(property write = node version write、既存どおり)。firstRel/firstProp/label は in-place 維持(エッジ作成は版を増やさない)、inline 値 set のみ copy-on-write。rel property は据え置き (Phase 4)。
