...
> 作成日: 2026-07-10
> 対応する正本のバージョン: 統合 commit I (`redesign/integration-c5-boundary` の本 merge commit)。
> ステータス: 承認済み(2026-07-12、C-7 legacy compatibility 統合)
...
- [ ] 正本 §2.3、§5.1、§8.1、§15、§16 と review C-7 の `LabelNodeIndex.Lookup` full ID、legacy raw-long compatibility adapter、validated `Sequence`、Wave 7 削除の規則が一致する。
...
1. typed ID equality を Generation 込みにする。
   ...
   `LabelNodeIndex.Lookup` は full `NodeId` を返す logical API とする。logical pipeline は full typed ID を保持し、physical store の locator、chain、index key へ渡す sequence は full ID の `Read` 検証後にだけ取り出す。physical candidate は full ID へ materialize して stale を skip する。public `EntityCandidateSet` と filtered vector の direct raw-long contract は Wave 1 で維持する互換例外だが、compatibility adapter 内の physical candidate にだけ使い、query/traversal へ渡す前に validated `Sequence` に限定する。Wave 7 でこの legacy contract を削除する。診断 raw `long` は表示・計測だけに閉じる。
