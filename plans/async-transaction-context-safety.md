...
状態: 停止（REF-16 による対象 API 撤回）

優先度: —

> **停止決定 (2026-07-02、Codex 実装現状レビュー)**: 非同期 tx API の撤回 (REF-16) により
> 本計画の対象 API が消滅するため、「停止 (やらない)」とする。本計画が要求する文脈の tx 所有化は
...
> tx 形 async を再導入検討する際の一次資料として削除しない。** REF-16 により対象 API を撤回したため、
> 本計画は停止した (FTS-9 と同じ流儀)。
>
> `CommitAsync` 単体は終端 API として技術的に分離できる可能性があるが、v1 では async 契約を
> `ExecuteWriteAsync` に一本化し、公開面と TCB を最小化する方針を優先して同時に撤回する。
