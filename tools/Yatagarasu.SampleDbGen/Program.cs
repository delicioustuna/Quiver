...
switch (mode)
{
    ...
    case "--portability-verify":
        ...
        break;
    case "--hierarchical":
        GenerateHierarchical(pathArg ?? Path.Combine(AppContext.BaseDirectory, "sample2.yata"));
        break;
    case "--vector":
        GenerateVector(pathArg ?? Path.Combine(AppContext.BaseDirectory, "sample3.yata"));
        break;
    default:
        GenerateMovie(pathArg ?? Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "sample", "movie.yata"));
        break;
}
...
static void PrintStats(YatagarasuDatabase db)
{
...
static void EditSchema(YatagarasuDatabase db, Action<ISchemaEditor> edit)
{
...
static void GenerateMovie(string outputPath)
{
    ...
    using var db = YatagarasuDatabase.Open(outputPath);
...
static void GenerateHierarchical(string outputPath)
{
    ...
    using (var tx = db.BeginWriteTransaction())
    {
        ...
        // --- 会社 (ルート) ---
        var company = tx.Mutate.AddVertex("Company").P("name", "Yatagarasu Corp").P("founded", 2024L).Next();
...
static void ProducePortabilityBundle(string bundlePath, string origin)
{
    ...
    string databasePath = Path.Combine(root, "graph.yata");
    ...
    var state = new PortabilityOracle(
        ...
        SnapshotRelativePath: "snapshot/stage-0/graph.yata");
...
static void VerifyAndUpdatePortabilityBundle(string bundlePath)
{
    ...
    using (var recoverySnapshot = YatagarasuDatabase.Open(
    ...
    using (var database = YatagarasuDatabase.Open(databasePath))
    {
        ...
        state = state with
        {
            ...
            SnapshotRelativePath = "snapshot/stage-1/graph.yata",
        };
        WaitForPortableArtifacts(database);
        AssertPortableDatabase(database, state);
        CreatePortableSnapshot(database, root, state.SnapshotRelativePath);
...
static void VerifyPortabilityBundle(string bundlePath, int expectedStage)
{
    ...
    using (var database = YatagarasuDatabase.Open(Path.Combine(root, "graph.yata")))
        AssertPortableDatabase(database, state);
    using (var snapshot = YatagarasuDatabase.Open(
               Path.Combine(root, state.SnapshotRelativePath.Replace('/', Path.DirectorySeparatorChar))))
...
static void WaitForPortableArtifacts(YatagarasuDatabase database)
{
...
static void AssertPortableDatabase(YatagarasuDatabase database, PortabilityOracle state)
{
