...
Quiver グラフデータベースを MCP (Model Context Protocol) サーバとして公開するコンソールアプリケーション。
...
```bash
# ソースから直接実行
dotnet run --project tools/Quiver.Mcp -- --config path/to/quiver-mcp.json

# ビルド済みバイナリで実行
dotnet publish tools/Quiver.Mcp -c Release -o ./publish
./publish/Quiver.Mcp --config path/to/quiver-mcp.json
```
...
claude mcp add quiver -- \
  dotnet run --project D:/csharp/Quiver/tools/Quiver.Mcp \
  -- --config D:/path/to/quiver-mcp.json
```
...
MCP クライアント (LM Studio / Claude)
    ...
    ▼
Quiver.Mcp プロセス
    ├── schema ツール ──→ ISchemaApi / IVectorStore
    └── traverse ツール ─→ IGraphTransaction (読み取り専用)
                           IChunkEmbedder (vector 検索時のみ)
    │
    ▼
Quiver エンジン (in-process)
    └── *.quiver ファイル
```
