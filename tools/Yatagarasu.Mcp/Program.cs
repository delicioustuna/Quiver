...
var dbPath = config.Databases.GetValueOrDefault("default")
    ?? throw new InvalidOperationException("No 'default' database configured in yatagarasu-mcp.json.");

// DB はプロセス起動時に 1 度だけ Open し、全リクエストで共有する (in-process 組み込み DB)
var db = YatagarasuDatabase.Open(dbPath);
