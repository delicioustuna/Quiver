// MCP サーバの設定モデル。
// 設定ファイル (yatagarasu-mcp.json) から DB パスや embedding エンドポイントを読み込む。
//
// 設定ファイルの探索順:
//   1. --config 引数で指定されたパス
//   2. カレントディレクトリの yatagarasu-mcp.json
//   3. ~/.yata/mcp.json (ユーザ共通設定)
// いずれも見つからなければ既定値 (カレントの data.yata) で起動する。
...
internal sealed class McpConfig
{
    ...
    public Dictionary<string, string> Databases { get; set; } = new() { ["default"] = "data.yata" };
    ...
    private static string? FindConfigFile()
    {
        var candidates = new[]
        {
            Path.Combine(Environment.CurrentDirectory, "yatagarasu-mcp.json"),
            Path.Combine(
