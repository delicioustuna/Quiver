...
//   Yatagarasu の PropertyValue は ref struct であり、ラムダキャプチャや
...
internal static class TraverseTool
{
    ...
    // startType に応じて Yatagarasu の検索 API を使い分ける。
    // LLM は type を指定するだけで、内部で機械的に適切な API に変換する。
    private static async Task<List<VertexId>> ResolveStartVertices(
        StartSpec start, YatagarasuDatabase db, OpenAiEmbedder? embedder)
    {
            ...
            case "vector":
            {
                ...
                if (embedder is null)
                    throw new InvalidOperationException(
                        "Embedding is not configured. Set 'embedding' in yatagarasu-mcp.json.");
    ...
    // label 検索の場合のみ、Yatagarasu の Gremlin 風 API (Has + P 述語) を直接使う。
