...
internal static class GraphRelationshipEmitter
{
    ...
    public static string Emit(GraphRelationshipModel model)
    {
        ...
        sb.AppendLine($"partial class {model.ClassName} : GraphDb.Engine.Client.IGraphRelationship<{model.ClassName}>");
        sb.AppendLine("{");
        sb.AppendLine($"    public static string GraphType => \"{model.RelType}\";");
        sb.AppendLine();

        // Insert
        sb.AppendLine($"    public static GraphDb.Engine.Core.RelationshipId Insert(IGraphTransaction tx, GraphDb.Engine.Core.NodeId from, GraphDb.Engine.Core.NodeId to, {model.ClassName} entity)");
        sb.AppendLine("    {");
        ...
        // Load
        sb.AppendLine($"    public static {model.ClassName} Load(IGraphTransaction tx, GraphDb.Engine.Core.RelationshipId id)");
        sb.AppendLine("    {");
        ...
        // Update
        sb.AppendLine($"    public static void Update(IGraphTransaction tx, GraphDb.Engine.Core.RelationshipId id, {model.ClassName} entity)");
        sb.AppendLine("    {");
        foreach (var prop in model.Properties)
            EmitSetProperty(sb, prop);
        sb.AppendLine("    }");
        ...
        // Delete
        sb.AppendLine($"    public static void Delete(IGraphTransaction tx, GraphDb.Engine.Core.RelationshipId id) => tx.DeleteRelationship(id);");
