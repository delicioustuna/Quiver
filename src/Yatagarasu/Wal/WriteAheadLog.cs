...
///     全データは graph.yata へ durable 済みなので、静止時はサイドカーが消えて本体のみが残る。</item>
/// </list>
...
internal sealed class WriteAheadLog : IWriteAheadLog
{
    ...
    /// <summary>
    /// クリーン終了の最終 flush 後に backend が呼ぶ。全データが graph.yata へ
    ...
    private long WriteRecordLocked(WalRecordType type, long txIdValue, ReadOnlySpan<byte> payload)
    {
        ...
        YatagarasuTelemetry.WalBytesWritten.Add(recordSize);
        YatagarasuEventSource.Log.WalBytesWritten(recordSize);
        return lsn;
    }
    ...
    public void FlushTo(long lsn)
    {
        ...
        using var activity = YatagarasuTelemetry.WalFlushActivitySource.StartActivity(
            "wal.flush", ActivityKind.Internal);
        activity?.SetTag("yatagarasu.wal.target_lsn", lsn);
        var sw = Stopwatch.StartNew();
        YatagarasuEventSource.Log.WalFlushRequestStarted();
        try
        {
            lock (_writeLock)
            {
            ...
            YatagarasuTelemetry.WalFlushDurationMs.Record(sw.Elapsed.TotalMilliseconds);
            YatagarasuEventSource.Log.WalFlushed(lsn, sw.Elapsed.TotalMilliseconds);
        }
        finally
        {
            YatagarasuEventSource.Log.WalFlushRequestCompleted();
        }
    }
    ...
    public void Dispose()
    {
        ...
        if (_deleteOnDispose)
        {
            // クリーン終了: データは graph.yata へ durable 済み。WAL は捨てるので最終フラッシュ不要。
            _disposed = true;
            lock (_writeLock)
            {
                _stream?.Dispose();
