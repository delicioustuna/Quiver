...
/// <summary>
/// Yatagarasu の最上位エントリポイント。グラフデータベースのオープン・トランザクション開始・
...
/// </remarks>
internal sealed class YatagarasuDatabase : IDisposable
{
    ...
    /// <summary><see cref="Open"/> に渡したデータベースファイルのパス (<c>*.yata</c>)。</summary>
    ...
    /// <summary>
    /// 指定した単一データベースファイル (<c>*.yata</c>) を開く (存在しない場合は新規作成)。
    /// Yatagarasu の binary backend は全データ (コア / 索引 / 隣接 / token / epoch) を
    /// 単一の <c>*.yata</c> ファイルに格納し、運用中のみサイドカー <c>*.yata-wal</c> を伴う。
    /// <paramref name="options"/> 経由でバックエンド種別やバッファプールサイズなどを指定可。
    /// </summary>
    /// <param name="filePath">データベースファイル (<c>*.yata</c>) のパス。</param>
    /// <param name="options">起動オプション。<c>null</c> の場合は既定値が使われる。</param>
    public static YatagarasuDatabase Open(string filePath, YatagarasuDatabaseOptions? options = null)
    {
        options ??= new YatagarasuDatabaseOptions();
        ...
        return new YatagarasuDatabase(backend, filePath, worker);
    }
    ...
    public static YatagarasuDatabase CreateInMemory(YatagarasuDatabaseOptions? options = null)
    {
        options ??= new YatagarasuDatabaseOptions();
        options.Backend = BackendKind.InMemory;
    ...
    /// <summary>
    /// 閉じた Yatagarasu データベースを、登録済みの手順で現行ストレージ形式へ移行する。
    ...
    /// <c>true</c> の場合、<see cref="BulkLoader.Commit"/> 時に隣接ブロックビュー (graph.yata
    ...
    internal SchemaApi SchemaApiForTesting
        => _backend switch
        {
            ...
            _ => throw new NotSupportedException(
                "The configured backend does not expose Yatagarasu's internal schema implementation."),
        };
    ...
    /// <summary>
    /// backendを閉じずに<paramref name="targetFilePath"/> (<c>*.yata</c>)へ
...
{
    /// <summary><see cref="YatagarasuDatabaseOptions.WriterWaitTimeout"/> まで待機する。</summary>
    Wait = 0,
...
/// <summary>
/// <see cref="YatagarasuDatabase.Open"/> に渡す起動オプション。
/// バッファプール、WAL、writer lease、チェックサム、バックエンド種別などを指定する。
/// </summary>
internal sealed class YatagarasuDatabaseOptions
{
    /// <summary>
    ...
    /// <see cref="CheckpointPolicy"/> が <see cref="Yatagarasu.Transactions.CheckpointPolicy.Adaptive"/>
    ...
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Fixed"/> (既定) は
    /// <see cref="CheckpointThresholdBytes"/> をそのまま使い続ける旧挙動。
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Adaptive"/> は直近
    /// <see cref="AdaptiveSampleWindow"/> 件の bytes/tx 移動平均から、
    /// <see cref="TargetRecoveryTime"/> を満たす threshold を周期的に再計算する。
    /// </summary>
    public Yatagarasu.Transactions.CheckpointPolicy CheckpointPolicy { get; set; }
        = Yatagarasu.Transactions.CheckpointPolicy.Fixed;

    /// <summary>
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Adaptive"/> 選択時の
    ...
    /// 周期的に <see cref="YatagarasuDatabase.Vacuum"/> を起動する。既定 <c>false</c>
    /// (運用者が明示的に <see cref="YatagarasuDatabase.Vacuum"/> を呼ぶ前提)。
    /// <c>true</c> かつ
    /// <see cref="AutoVacuumInterval"/> が正のとき、<see cref="YatagarasuDatabase.Open"/> が
    /// <see cref="Yatagarasu.Maintenance.AutoVacuumWorker"/> を起動し、
    /// <see cref="YatagarasuDatabase.Dispose"/> で停止する。
    /// </summary>
