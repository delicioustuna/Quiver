...
/// <c>buildAdjacencyIndex: false</c> を選び、必要なら後段で <c>YatagarasuDatabase.CompactAdjacency()</c> を
/// 呼ぶことを推奨する。
/// </summary>
internal sealed class StreamingBulkLoader : IDisposable
{
    ...
    // 隣接ビューは graph.yata 内テナントへ構築する (null = 構築しない)。
    private readonly Yatagarasu.Storage.SingleFileContainer? _container;
    ...
    internal StreamingBulkLoader(
        ...
        Yatagarasu.Storage.SingleFileContainer? container = null,
        ...
        Action? afterCommit = null)
    {
        ...
        _tempPath = Path.Combine(
            Path.GetTempPath(),
            "yatagarasu_streaming_bulk_" + Guid.NewGuid().ToString("N")[..16] + ".edges");
    ...
    private void BuildAdjacencyIndexStreaming(Yatagarasu.Storage.SingleFileContainer container)
    {
