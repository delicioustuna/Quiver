...
/// すべてのページを単一 <c>graph.yata</c> に同居させる:
/// <list type="bullet">
...
internal sealed class IndexManager : IIndexManager, IDisposable
{
    ...
    /// <c>graph.yata</c> コンテナを作成 (または開いて) その上に索引テナントを載せた
    ...
    internal static IndexManager OpenStandalone(string directory)
    {
        ...
        var container = new SingleFileContainer(Path.Combine(directory, "graph.yata"));
        try
        {
    ...
    /// <see cref="Yatagarasu.Transactions.Checkpointer"/> がチェックポイント時に呼び、
    ...
    public bool DropIndex(string name)
    {
        ...
        // (graph.yata 自体は縮まないが、解放ページは他テナントへ再割当できる)。
    ...
    /// 索引は <c>graph.yata</c> に同居するようになったため、snapshot の page-by-page コピーは
