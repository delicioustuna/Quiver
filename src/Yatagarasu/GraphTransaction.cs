...
internal sealed class GraphTransaction : IWriteTransaction, IReadTransactionInternal
{
            ...
            if (_propKeyTokens.TryGet(matchKey, out var keyId))
            {
                if (_mergeFullscanWarned.TryAdd((label, matchKey), 0))
                {
                    System.Diagnostics.Trace.TraceWarning(
                        "Yatagarasu.MergeVertex: (label='{0}', key='{1}') にインデックスが未登録のためフルスキャンに落ちました。" +
    ...
    public QueryResult Execute(IPhysicalOperator plan)
    {
        ...
        try
        {
            ...
            using var activity = YatagarasuTelemetry.QueryActivitySource.StartActivity(
                "query.execute", ActivityKind.Internal);
            activity?.SetTag("yatagarasu.tx.id", _inner.Id.Value);
            ...
            plan.Dispose();
            YatagarasuTelemetry.QueryDurationMs.Record(sw.Elapsed.TotalMilliseconds);
            activity?.SetTag("yatagarasu.query.rows", rows.Count);
            // EventSource で件数 + 経過時間を発行。N+1 検出や hot operator 推定に有効。
            YatagarasuEventSource.Log.QueryExecuted(
                _inner.Id.Value,
                rows.Count,
