...
/// <see cref="YatagarasuDatabase.Diagnostics"/> から取得する。
/// </summary>
public interface IDiagnosticsApi
{
    ...
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Fixed"/> 時は起動時に与えた値、
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Adaptive"/> 時は controller が
    ...
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Fixed"/> 時は
    /// <paramref name="fixedThresholdBytes"/> を新しい固定 threshold として採用。
    /// <see cref="Yatagarasu.Transactions.CheckpointPolicy.Adaptive"/> 時は
    ...
    void SetCheckpointPolicy(
        Yatagarasu.Transactions.CheckpointPolicy policy,
...
/// <summary>
/// orphan 索引エントリ。<paramref name="EntityId"/> は <see cref="Yatagarasu.Core.VertexId.Value"/>
