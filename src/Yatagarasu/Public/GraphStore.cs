...
public sealed class GraphOperationException : YatagarasuException
{
...
public sealed class GraphStoreOptions
{
    ...
    internal YatagarasuDatabaseOptions ToCore(bool inMemory)
    {
        return new YatagarasuDatabaseOptions
        {
            ...
            WriterContentionMode = WriterContentionMode == GraphWriterContentionMode.FailFast
                ? global::Yatagarasu.WriterContentionMode.FailFast
                : global::Yatagarasu.WriterContentionMode.Wait,
...
public sealed class AdvancedGraphStore
{
    ...
    internal AdvancedGraphStore(YatagarasuDatabase database) => _database = database;
...
/// <summary>
/// Yatagarasu の既定エントリポイント。通常操作を同期 callback 内へ閉じ込め、
...
public sealed class GraphStore : IDisposable
{
    ...
    private GraphStore(YatagarasuDatabase database)
    {
        _database = database;
        Advanced = new AdvancedGraphStore(database);
    }

    /// <summary>単一の <c>*.yata</c> ファイルを開く。存在しない場合は作成する。</summary>
    public static GraphStore Open(string filePath, GraphStoreOptions? options = null)
    {
        ArgumentException.ThrowIfNullOrEmpty(filePath);
        options ??= new GraphStoreOptions();
        return new GraphStore(YatagarasuDatabase.Open(filePath, options.ToCore(inMemory: false)));
    }
    ...
    {
        options ??= new GraphStoreOptions();
        return new GraphStore(YatagarasuDatabase.CreateInMemory(options.ToCore(inMemory: true)));
    }
    ...
    internal YatagarasuDatabase DatabaseInternal => _database;
