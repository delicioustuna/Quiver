...
internal sealed class Transaction : ITransaction
{
    ...
    public void Commit()
    {
        ...
        using Activity? activity = YatagarasuTelemetry.TransactionActivitySource.StartActivity(
            "tx.commit",
            ActivityKind.Internal);
        activity?.SetTag("yatagarasu.tx.id", Id.Value);
    ...
    private void AbortCore()
    {
        ...
        if (!_isReadOnly)
        {
            YatagarasuTelemetry.TxAbortCount.Add(1);
            YatagarasuTelemetry.TxAbortDurationMs.Record(stopwatch.Elapsed.TotalMilliseconds);
            YatagarasuEventSource.Log.TxAborted(Id.Value, stopwatch.Elapsed.TotalMilliseconds);
        }
        FireHooks(_onRolledBack);
    }
    ...
    {
        YatagarasuTelemetry.TxCommitCount.Add(1);
        YatagarasuTelemetry.TxCommitDurationMs.Record(stopwatch.Elapsed.TotalMilliseconds);
        YatagarasuEventSource.Log.TxCommit();
        YatagarasuEventSource.Log.TxCommitted(Id.Value, stopwatch.Elapsed.TotalMilliseconds);
        activity?.SetStatus(
        ...
        Exception exception)
    {
        YatagarasuTelemetry.TxAbortCount.Add(1);
        YatagarasuTelemetry.TxAbortDurationMs.Record(stopwatch.Elapsed.TotalMilliseconds);
        YatagarasuEventSource.Log.TxAbort();
        YatagarasuEventSource.Log.TxCommitFailed(
            Id.Value,
            exception.Message,
