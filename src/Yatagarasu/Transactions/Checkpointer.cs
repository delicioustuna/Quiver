    ...
    Action? prepareCheckpoint = null)
{
    ...
    public void Checkpoint()
    {
        ...
        using var activity = YatagarasuTelemetry.CheckpointActivitySource.StartActivity(
        ...
        YatagarasuTelemetry.CheckpointDurationMs.Record(sw.Elapsed.TotalMilliseconds);
        activity?.SetTag("yatagarasu.checkpoint.begin_lsn", beginLsn);
        // 運用上「実際にチェックポイントが完走した」マイルストーンとして残す。
        YatagarasuEventSource.Log.CheckpointCompleted(beginLsn, sw.Elapsed.TotalMilliseconds);
    }
}
