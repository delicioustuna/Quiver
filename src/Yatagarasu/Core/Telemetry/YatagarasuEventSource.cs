...
/// <summary>
/// <c>dotnet-counters monitor -n &lt;pid&gt; --counters Yatagarasu-EventSource</c> で
...
///    Vacuum) が直接インクリメント API を呼ぶ。複数 <c>YatagarasuDatabase</c> インスタンスが
...
/// </list>
/// </remarks>
[EventSource(Name = "Yatagarasu-EventSource")]
internal sealed class YatagarasuEventSource : EventSource
{
    internal const string EventSourceName = "Yatagarasu-EventSource";

    /// <summary>プロセス全体の singleton。各 hot path はここから直接インクリメント API を呼ぶ。</summary>
    public static readonly YatagarasuEventSource Log = new();
    ...
    /// <summary>crash recovery が起動した (1 回 = 1)。<see cref="Yatagarasu.Transactions.RecoveryManager.Recover"/> で 1 度呼ぶ。</summary>
    [NonEvent]
