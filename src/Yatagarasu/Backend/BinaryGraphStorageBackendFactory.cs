...
/// <summary>
/// Default factory used when <see cref="YatagarasuDatabaseOptions.Backend"/> is
...
internal sealed class BinaryGraphStorageBackendFactory : IGraphStorageBackendFactory
{
    ...
    public IGraphStorageBackend Open(string filePath, YatagarasuDatabaseOptions options)
    {
        ...
        // filePath は単一コンテナ (*.yata) のフルパス。親ディレクトリを用意する。
        ...
        // epoch をすべて *.yata に同居させ、全ページを単一 DATA fileKind で WAL に載せる。
        // 物理ページ ID は全テナント横断で一意なので recovery / abort は純物理ページ単位で動く。
        // YatagarasuDatabaseOptions.BufferPoolSize を共有プール容量に実配線する。
    ...
    /// </summary>
    internal IGraphStorageBackend OpenInMemory(YatagarasuDatabaseOptions options)
    {
        var pageManager = new PageManager();
    ...
    private static IGraphStorageBackend OpenCore(
        string filePath,
        YatagarasuDatabaseOptions options,
        ...
        bool initializeDatabaseIdentity)
    {
        ...
        if (recover)
        {
            ...
            if (options.CheckpointPolicy == Yatagarasu.Transactions.CheckpointPolicy.Adaptive
