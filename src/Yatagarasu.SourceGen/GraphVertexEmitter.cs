...
internal static class GraphVertexEmitter
{
    ...
    {
        ["string"]  = "Yatagarasu.Storage.Records.PropertyValue.FromString(value)",
        ["string?"] = "Yatagarasu.Storage.Records.PropertyValue.FromString(value ?? \"\")",
        ["int"]     = "Yatagarasu.Storage.Records.PropertyValue.FromInt64((long)value)",
        ["long"]    = "Yatagarasu.Storage.Records.PropertyValue.FromInt64(value)",
        ["double"]  = "Yatagarasu.Storage.Records.PropertyValue.FromDouble(value)",
    };

    private static readonly Dictionary<string, string> _indexKindMap = new()
    {
        ["string"]  = "Yatagarasu.IndexKind.StringEquality",
        ["string?"] = "Yatagarasu.IndexKind.StringEquality",
        ["int"]     = "Yatagarasu.IndexKind.Int32Equality",
        ["long"]    = "Yatagarasu.IndexKind.Int64Equality",
        ["double"]  = "Yatagarasu.IndexKind.DoubleEquality",
    };
    ...
    private static readonly Dictionary<string, (string write, string read)> _publicTypeMap = new()
    {
        ...
        ["float"]   = ("Yatagarasu.GraphValue.FromDouble((double)entity.{0})", "(float)read.Get(vertex, \"{1}\").AsDouble()"),
        ["Half"]    = ("Yatagarasu.GraphValue.FromDouble((double)entity.{0})", "(System.Half)read.Get(vertex, \"{1}\").AsDouble()"),
        ["bool"]    = ("entity.{0}", "read.Get(vertex, \"{1}\").AsBoolean()"),
        ["DateTime"]       = ("Yatagarasu.GraphValue.FromDateTime(entity.{0})", "read.Get(vertex, \"{1}\").AsDateTime()"),
        ["DateTimeOffset"] = ("Yatagarasu.GraphValue.FromDateTimeOffset(entity.{0})", "read.Get(vertex, \"{1}\").AsDateTimeOffset()"),
        ["DateOnly"]       = ("Yatagarasu.GraphValue.FromDateOnly(entity.{0})", "read.Get(vertex, \"{1}\").AsDateOnly()"),
        ["TimeOnly"]       = ("Yatagarasu.GraphValue.FromTimeOnly(entity.{0})", "read.Get(vertex, \"{1}\").AsTimeOnly()"),
        ["TimeSpan"]       = ("Yatagarasu.GraphValue.FromTimeSpan(entity.{0})", "read.Get(vertex, \"{1}\").AsTimeSpan()"),
        ["float[]"]        = ("Yatagarasu.GraphValue.FromFloatVector(entity.{0})", "read.Get(vertex, \"{1}\").AsFloatVector().ToArray()"),
    };
    ...
    public static string Emit(GraphVertexModel model)
    {
        ...
        sb.AppendLine($"partial class {model.ClassName} : Yatagarasu.Api.IGraphVertex<{model.ClassName}>, Yatagarasu.IGraphEntity<{model.ClassName}>, Yatagarasu.IGraphVertexSchema<{model.ClassName}>");
        sb.AppendLine("#else");
        sb.AppendLine($"partial class {model.ClassName} : Yatagarasu.IGraphEntity<{model.ClassName}>, Yatagarasu.IGraphVertexSchema<{model.ClassName}>");
        sb.AppendLine("#endif");
        sb.AppendLine("{");
        ...
        // 挿入
        sb.AppendLine($"    public static Yatagarasu.Core.VertexId Insert(IWriteTransaction tx, {model.ClassName} entity)");
        sb.AppendLine("    {");
        ...
        // インデックス付き挿入
        sb.AppendLine($"    public static Yatagarasu.Core.VertexId InsertIndexed(IWriteTransaction tx, {model.ClassName} entity)");
        sb.AppendLine("    {");
        ...
        // 読み込み
        sb.AppendLine($"    public static {model.ClassName} Load(IReadTransaction tx, Yatagarasu.Core.VertexId id)");
        sb.AppendLine("    {");
        ...
        // 更新
        sb.AppendLine($"    public static void Update(IWriteTransaction tx, Yatagarasu.Core.VertexId id, {model.ClassName} entity)");
        sb.AppendLine("    {");
        foreach (var prop in model.Properties)
        {
            if (prop.IsMultiValued)
        ...
        sb.AppendLine($"    public static Yatagarasu.VertexGraphRewriteResult Replace(IWriteTransaction tx, Yatagarasu.Core.VertexId id, {model.ClassName} entity)");
        sb.AppendLine("    {");
        ...
        // 削除
        sb.AppendLine($"    public static void Delete(IWriteTransaction tx, Yatagarasu.Core.VertexId id) => tx.DeleteVertex(id);");
        sb.AppendLine("#endif");
        ...
        // 全インデックスの作成保証
        sb.AppendLine();
        sb.AppendLine("    public static void EnsureIndexes(Yatagarasu.ISchemaEditor schema)");
        sb.AppendLine("    {");
        foreach (var prop in indexedProps)
        {
            ...
            sb.AppendLine(
                $"        schema.CreateIndex(new Yatagarasu.ScalarIndexDefinition(\"{prop.IndexName}\", new Yatagarasu.PropertyTarget(Yatagarasu.PropertyOwnerKind.Vertex, \"{prop.GraphKey}\", \"{model.Label}\"), {kindExpr}, {uniqueExpr}));");
        }
        sb.AppendLine("    }");

        // 単一プロパティのインデックス作成保証
        sb.AppendLine();
        sb.AppendLine("    public static void EnsureIndex(Yatagarasu.ISchemaEditor schema, string propertyName, Yatagarasu.IndexKind? kindOverride)");
        sb.AppendLine("    {");
        ...
        foreach (var prop in indexedProps)
        {
            ...
            sb.AppendLine(
                $"                schema.CreateIndex(new Yatagarasu.ScalarIndexDefinition(\"{prop.IndexName}\", new Yatagarasu.PropertyTarget(Yatagarasu.PropertyOwnerKind.Vertex, \"{prop.GraphKey}\", \"{model.Label}\"), kindOverride ?? {kindExpr}, {uniqueExpr}));");
        ...
        foreach (var prop in indexedProps)
        {
            ...
            sb.AppendLine();
            sb.AppendLine($"    public static System.Collections.Generic.List<(Yatagarasu.Core.VertexId Id, {model.ClassName} Entity)> FindBy{prop.PropertyName}(");
            sb.AppendLine($"        IReadTransaction tx, {paramType} value)");
            sb.AppendLine("    {");
            sb.AppendLine($"        var results = new System.Collections.Generic.List<(Yatagarasu.Core.VertexId, {model.ClassName})>();");
            sb.AppendLine($"        var seek = tx.SeekIndex(\"{prop.IndexName}\", {seekExpr});");
            sb.AppendLine("        while (seek.MoveNext())");
            sb.AppendLine("        {");
            sb.AppendLine("            if (seek.Current.Kind != Yatagarasu.Core.EntityKind.Vertex) continue;");
            sb.AppendLine("            var id = new Yatagarasu.Core.VertexId(seek.Current.Value);");
    ...
    {
        sb.AppendLine();
        sb.AppendLine($"    public static {model.ClassName} Read(Yatagarasu.GraphReadAccess read, Yatagarasu.VertexKey vertex)");
        sb.AppendLine("    {");
        sb.AppendLine($"        var __entity = new {model.ClassName}");
        sb.AppendLine("        {");
        foreach (var prop in model.Properties)
        ...
        sb.AppendLine();
        sb.AppendLine($"    public static void Write(Yatagarasu.GraphWriteAccess write, Yatagarasu.VertexKey vertex, {model.ClassName} entity)");
        sb.AppendLine("    {");
        foreach (var prop in model.Properties)
        {
            if (prop.IsMultiValued)
