...
/// <summary>
/// Yatagarasu core の process-wide EventSource イベントを Microsoft.Extensions.Logging へ転送する。
/// </summary>
/// <remarks>
/// EventSource 自体がプロセス単位なので、イベントは特定の <c>YatagarasuDatabase</c> インスタンスには
/// ルーティングされない。通常の Generic Host 構成では本 listener を 1 個だけ生成する。
/// </remarks>
internal sealed class YatagarasuEventLoggerBridge : EventListener
{
    private const string SourceName = "Yatagarasu-EventSource";
    private const string TransactionCategory = "Yatagarasu.Transaction";
    private const string QueryCategory = "Yatagarasu.Query";
    private const string CheckpointCategory = "Yatagarasu.Checkpoint";
    private const string WalCategory = "Yatagarasu.Wal";
    ...
    public YatagarasuEventLoggerBridge(ILoggerFactory? loggerFactory)
    {
        ...
        foreach (var source in EventSource.GetSources())
            EnableIfYatagarasu(source);
    }

    protected override void OnEventSourceCreated(EventSource eventSource)
    {
        if (_transactionLogger is not null)
            EnableIfYatagarasu(eventSource);
    }

    private void EnableIfYatagarasu(EventSource eventSource)
    {
    ...
    private void LogTransactionCompleted(EventWrittenEventArgs data, bool committed)
    {
        ...
        using var scope = _transactionLogger!.BeginScope(
            LogState.Scope(("yatagarasu.tx.id", txId), ("yatagarasu.op", operation)));
        Log(
            _transactionLogger,
            level,
            eventId,
    ...
    private void LogTransactionFailure(EventWrittenEventArgs data)
    {
        ...
        using var scope = _transactionLogger!.BeginScope(
            LogState.Scope(("yatagarasu.tx.id", txId), ("yatagarasu.op", "Commit")));
        Log(
            _transactionLogger,
            LogLevel.Error,
            3,
    ...
    private void LogQuery(EventWrittenEventArgs data)
    {
        ...
        using var scope = _queryLogger!.BeginScope(
            LogState.Scope(("yatagarasu.tx.id", txId), ("yatagarasu.op", "Execute")));
        Log(
            _queryLogger,
            LogLevel.Debug,
            30,
