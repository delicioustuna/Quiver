...
/// <summary>
/// <see cref="IServiceCollection"/> に Quiver の <see cref="QuiverDatabase"/> シングルトンを
...
/// そのままキーになる。コンテナ破棄時に <see cref="QuiverDatabase.Dispose"/> が呼ばれる。
/// </remarks>
public static class QuiverServiceCollectionExtensions
{
    /// <summary>
    /// <paramref name="configuration"/> セクション (例: <c>config.GetSection("Quiver")</c>) を
    /// <see cref="QuiverConfigurationOptions"/> に bind し、<see cref="QuiverDatabase"/> を
    /// シングルトンとして登録する。
    /// </summary>
    ...
    /// bind 後の <see cref="QuiverDatabaseOptions"/> をさらに編集するための任意フック。
    /// <see cref="QuiverDatabaseOptions.BackendFactory"/> や
    /// <see cref="QuiverDatabaseOptions.LogicalMutationSink"/> のようにバインダで表現できない
    /// メンバを差し込むのに使う。
    /// </param>
    public static IServiceCollection AddQuiver(
        this IServiceCollection services,
        IConfiguration configuration,
        Action<QuiverDatabaseOptions>? postConfigure = null)
    {
        ArgumentNullException.ThrowIfNull(services);
        ArgumentNullException.ThrowIfNull(configuration);

        services.AddOptions<QuiverConfigurationOptions>().Bind(configuration);
        RegisterQuiverDatabase(services, postConfigure);
        return services;
    }
    ...
    private static void RegisterQuiverDatabase(
        IServiceCollection services,
        Action<QuiverDatabaseOptions>? postConfigure)
    {
        services.TryAddSingleton(
            sp => new QuiverEventLoggerBridge(sp.GetService<Microsoft.Extensions.Logging.ILoggerFactory>()));
        services.AddSingleton<QuiverDatabase>(sp =>
        {
