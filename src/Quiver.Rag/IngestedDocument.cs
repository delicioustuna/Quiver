...
/// <summary>
/// 取込側 (PdfTools 等) が生成する正規化済みドキュメント。
/// <see cref="Blocks"/> は正しい読み順で並んでいることが契約 (レイアウト解析・読み順復元は取込側の責務)。
...
/// チャンカーはこの境界を尊重してチャンク化する (ブロック跨ぎ分割を避ける)。
/// </summary>
...
/// <summary>
/// 正規化ブロックの種別。チャンカーの分割規則 (Table / Code は分割しない 等) を分ける。
/// </summary>
