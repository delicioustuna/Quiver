...
/// <summary>
/// ARCH-3: B+Tree 索引が返す世代付きパック値 (<see cref="GenerationalRef"/>) を <c>NodeId.Value</c> へ
...
internal static class IndexValueResolver
{
    ...
    public static IEnumerable<long> ResolveLiveNodeSequences(IEnumerable<long> packedValues, INodeStore nodes)
    {
        ...
        {
            if (GenerationalRef.Kind(packed) != EntityKind.Node) continue;
            long seq = GenerationalRef.Sequence(packed);
            if (nodes.CurrentGeneration(seq) == GenerationalRef.Generation(packed))
                yield return seq;
        }
    }
