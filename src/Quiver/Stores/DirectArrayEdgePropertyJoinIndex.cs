...
/// <summary>
/// 実装 (a): <see cref="EdgeId"/> をインデックスとする dense 直接配列。
/// Edge slot あたり <see cref="long"/> 1 個 + presence ビット 1 個を確保するため、
/// 占有率に関わらずメモリは slot あたり約 9 バイト。Edge ID が密な場合
/// (bulk-load / append-only ケース) でインデックス対象キーが大半のエッジをカバーする場合に適する。
/// </summary>
/// <remarks>
/// 疎な場合 (Edgeの一部のみにキーが設定) でも slot コストは発生する。エントリ数が
/// 閾値を下回った場合に <see cref="Dictionary{TKey, TValue}"/> へフォールバックする変種は将来課題。
/// 現状は dense 形式のみ — 重み付きトラバーサルでは「全エッジが重みを持つ」のが典型パターンのため。
///
/// 型不一致は欠落扱い — <see cref="IEdgePropertyJoinIndex.KeyId"/> のプロパティ値が
/// インデックス構築時と異なる <see cref="PropertyValueType"/> を持つEdgeはスキップされ、
/// 型不一致 = 述語 false の慣習に従う。
/// </remarks>
internal sealed class DirectArrayEdgePropertyJoinIndex : IEdgePropertyJoinIndex
{
    ...
    private DirectArrayEdgePropertyJoinIndex(
        ...
        out long scalarBits)
    {
        ...
        long id = edgeId.Sequence; // dense 配列 index は Sequence
    ...
    /// <summary>
    /// <paramref name="edgeStore"/> 内のすべての生存中Edgeを走査し、
    ...
    /// </param>
    public static DirectArrayEdgePropertyJoinIndex Build(
        ...
        PropertyValueType expectedType)
    {
        ...
        // slot 数: 最大の live edgeId + 1。HWM を発見するために一度走査し、
        // 値収集パスの前に配列を適切なサイズに確保する。
        long hwm = 0;
        foreach (var edgeId in edgeStore.Scan())
            if (edgeId.Sequence + 1 > hwm) hwm = edgeId.Sequence + 1; // 配列サイズは Sequence

        if (hwm > int.MaxValue)
        {
            throw new NotSupportedException(
                $"DirectArrayEdgePropertyJoinIndex caps at int.MaxValue edges (saw hwm={hwm}).");
        }
        ...
        foreach (var edgeId in edgeStore.Scan())
        {
            // scalar 値は edge record へ inline されるため、join index も
            // inline + overflow を結合列挙する (join index 対象型はすべて inline 対象)。
            var pe = edgeStore.EnumerateProperties(edgeId, propStore);
            while (pe.MoveNext())
            {
                var cur = pe.Current;
                ...
                int idx = (int)edgeId.Sequence; // dense 配列 index は Sequence
                bits[idx] = raw;
        ...
        return new DirectArrayEdgePropertyJoinIndex(keyId, expectedType, bits, presence, entryCount);
    }
}
