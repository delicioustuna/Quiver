...
///
/// ペイロード形式 (FTS-9 で <c>leafPageId</c> を追加):
/// <code>
///   [op:1] [indexTenantId:1] [leafPageId:8 LE] [keyLen:2 LE] [keyBytes:keyLen] [value:8 LE]
///     op            = 1: Upsert / 2: Delete
///     indexTenantId = postings or norms の tenant byte (logical FT 索引の識別; §10.8)
///     leafPageId    = runtime で当該キーが書かれた leaf の PageId (FTS-9: recovery の page-targeted
///                     redo がこのページへ直接適用し traversal を排除する; design 13 §11.3)。
...
internal static class FtLeafMutationCodec
{
    ...
    private const int HeaderLen = 1 + 1 + 8 + 2; // op + tenant + leafPageId + keyLen

    /// <summary>ペイロードをエンコードする。</summary>
    public static byte[] Encode(Op op, byte indexTenantId, long leafPageId, ReadOnlySpan<byte> key, long value)
    {
        ...
        buf[0] = (byte)op;
        buf[1] = indexTenantId;
        BinaryPrimitives.WriteInt64LittleEndian(buf.AsSpan(2), leafPageId);
        BinaryPrimitives.WriteUInt16LittleEndian(buf.AsSpan(10), (ushort)key.Length);
    ...
    public static bool TryDecode(
        ...
        out Op op, out byte indexTenantId, out long leafPageId, out ReadOnlySpan<byte> key, out long value)
    {
        op = default;
        indexTenantId = 0;
        leafPageId = 0;
        ...
        leafPageId = BinaryPrimitives.ReadInt64LittleEndian(payload[2..]);
        int keyLen = BinaryPrimitives.ReadUInt16LittleEndian(payload[10..]);
