...
/// </summary>
public sealed class StorageFormatMismatchException : QuiverException
{
...
public sealed class WalFormatMismatchException : QuiverException
{
    ...
    public WalFormatMismatchException(string found, string expected)
        : base($"WAL format mismatch: found {found}, expected {expected}. " +
               "This Quiver build does not read WAL files from another format family.")
    {
        Found = found;
        Expected = expected;
    }
