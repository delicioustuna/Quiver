...
| 制限 | 概要 | 緩和策 |
|---|---|---|
| **単一ライタ** | 書き込みトランザクションは同時に 1 つのみ。トランザクションハンドルは同時使用不可 | 内蔵 writer gate を使い、必要に応じて専用ライタキューでバッチ化 |
...
| **Derived vector rebuild** | reopen 直後や segment merge 中は primary property の exact scan を使う | バックグラウンド merge 完了後に immutable HNSW segment へ切り替わる |
