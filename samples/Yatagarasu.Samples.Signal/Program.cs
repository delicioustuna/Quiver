// Yatagarasu.Samples.Signal — ApplyDyadic による float[] の演算処理。
//
// デモの流れ: テンプレート波形 → トラバーサルによるフィルタ → CosineSimilarityOp によるランキング
// → メタデータ取得. 演算範囲指定およびサブトラバーサル引数のサンプルも記載する。
//
// 実行: dotnet run --project samples/Yatagarasu.Samples.Signal
...
string dir = Path.Combine(Path.GetTempPath(), "yatagarasu_signal_" + Guid.NewGuid().ToString("N")[..8]);
try
{
