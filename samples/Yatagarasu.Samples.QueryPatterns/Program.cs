// Yatagarasu.Samples.QueryPatterns — 読み取り/書き込みクエリパターン集。
//
// 実行: dotnet run --project samples/Yatagarasu.Samples.QueryPatterns
//
...
using Yatagarasu.Samples.QueryPatterns;

string dir = Path.Combine(Path.GetTempPath(), "yatagarasu_qp_" + Guid.NewGuid().ToString("N")[..8]);
try
{
    ...
    using (var tx = db.BeginWriteTransaction())
    {
        ...
        var (eveId, eveCreated) = tx.Mutate.MergeVertex("Person", "Name", Yatagarasu.Storage.Records.PropertyValue.FromString("Eve"));
        if (eveCreated)
            tx.SetProperty(eveId, "Age", Yatagarasu.Storage.Records.PropertyValue.FromInt32(22));
