// Yatagarasu.Samples.Nexuses — 第一級Nexusによる RAG n 項ファクト。
// Yatagarasu.Rag の取込で得た出典チャンクをロール source に束ねた 4 ロールの Fact を作り、
// 同じファクトを型なし DSL / Match / 型付き API の三経路で読み戻して、
// 最後に出典チャンクの本文を回収する。
//
// 実行: dotnet run --project samples/Yatagarasu.Samples.Nexuses
...
using Yatagarasu.Samples.Nexuses;

string dir = Path.Combine(Path.GetTempPath(), "yatagarasu_nexuses_" + Guid.NewGuid().ToString("N")[..8]);
try
{
    using var graphStore = GraphStore.Open(Path.Combine(dir, "graph.yata"));
    var db = graphStore.DatabaseInternal;

    // ── 0. Yatagarasu.Rag で文書を取込み、出典チャンクを特定する ──
    ...
    await store.UpsertDocumentAsync(new IngestedDocument(
        ...
        [
            new IngestedBlock(BlockKind.Paragraph, "Acme 社は 2026 年にグラフエンジン Yatagarasu を導入した"),
    ...
    VertexId acme, yatagarasu, graphdb, y2026;
    NexusId verifiedFact;
    using (var tx = db.BeginWriteTransaction())
    {
        acme    = Entity.Insert(tx, new Entity { Name = "Acme" });
        yatagarasu  = Entity.Insert(tx, new Entity { Name = "Yatagarasu" });
        graphdb = Entity.Insert(tx, new Entity { Name = "YatagarasuDatabase" });
        ...
        verifiedFact = Fact.Insert(tx, new Fact
        {
            ...
            Objects = [new GraphVertexRef<Entity>(yatagarasu)],
    ...
    {
        [acme] = "Acme", [yatagarasu] = "Yatagarasu", [graphdb] = "YatagarasuDatabase",
