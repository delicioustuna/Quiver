...
static VertexId Service(IWriteTransaction write, string name)
{
    ...
    write.SetProperty(service, "name", Quiver.Storage.Records.PropertyValue.FromString(name));
    return service;
}
    ...
    params VertexId[] providers)
{
    ...
    write.SetProperty(requirement, "name", Quiver.Storage.Records.PropertyValue.FromString(name));
}
